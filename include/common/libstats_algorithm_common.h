...
namespace algorithm_utils {
    ...
    inline auto safe_min_element(Iterator first, Iterator last) -> Iterator {
        return std::min_element(first, last, [](const auto& a, const auto& b) {
            ...
            if (std::isnan(a) && std::isnan(b)) return false;  // NaN == NaN for min purposes
            if (std::isnan(a)) return false;  // NaN is not minimum
            if (std::isnan(b)) return true;   // b is NaN, a is minimum
            return a < b;
        });
    }
    ...
    inline auto safe_max_element(Iterator first, Iterator last) -> Iterator {
        return std::max_element(first, last, [](const auto& a, const auto& b) {
            // Handle NaN: NaN is considered greater than any finite value
            if (std::isnan(a) && std::isnan(b)) return false;  // NaN == NaN for max purposes
            if (std::isnan(a)) return true;   // NaN is maximum
            if (std::isnan(b)) return false;  // b is NaN, a is not maximum
            return a < b;
        });
    }
    ...
    inline std::size_t count_finite(Iterator first, Iterator last) {
        return std::count_if(first, last, [](const auto& value) {
    ...
    inline void statistical_sort(Iterator first, Iterator last) {
        ...
        std::sort(first, last, [](const auto& a, const auto& b) {
            if (std::isnan(a) && std::isnan(b)) return false;  // NaN == NaN
    ...
    namespace statistical {
        ...
        inline OutputIterator find_outliers_iqr(Iterator first, Iterator last, 
                                               OutputIterator out, double multiplier = 1.5) {
            ...
            if (size < 4) return out;  // Need at least 4 points for IQR
