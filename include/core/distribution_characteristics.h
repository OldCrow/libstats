...
constexpr std::array<DistributionComplexity, 6> DISTRIBUTION_CHARACTERISTICS = {{
    ...
    // POISSON: Iterative algorithms (Knuth's algorithm for small lambda, acceptance-rejection for large)
    ...
    // GAMMA: Acceptance-rejection sampling (Marsaglia & Tsang for shape >= 1, other methods for shape < 1)
...
constexpr const DistributionComplexity& getCharacteristics(DistributionType dist_type) noexcept {
    switch (dist_type) {
        case DistributionType::UNIFORM: return DISTRIBUTION_CHARACTERISTICS[0];
        case DistributionType::GAUSSIAN: return DISTRIBUTION_CHARACTERISTICS[1];
        case DistributionType::EXPONENTIAL: return DISTRIBUTION_CHARACTERISTICS[2];
        case DistributionType::DISCRETE: return DISTRIBUTION_CHARACTERISTICS[3];
        case DistributionType::POISSON: return DISTRIBUTION_CHARACTERISTICS[4];
        case DistributionType::GAMMA: return DISTRIBUTION_CHARACTERISTICS[5];
    }
...
namespace scaling {
     ...
     */
    constexpr double calculateParallelSpeedup(const DistributionComplexity& chars, size_t num_threads) noexcept {
...
namespace adaptive {
     ...
     */
    constexpr DistributionComplexity applyRefinements(
        const DistributionComplexity& base_chars,
        const LearnedRefinements& refinements
    ) noexcept {
        ...
        return DistributionComplexity{
            ...
            .vectorization_efficiency = base_chars.vectorization_efficiency * 
                                       (1.0 - blend_factor + blend_factor * refinements.simd_efficiency_multiplier),
            .parallelization_efficiency = base_chars.parallelization_efficiency * 
            ...
            .min_simd_threshold = static_cast<size_t>(
                ...
                static_cast<double>(base_chars.min_simd_threshold + refinements.simd_threshold_offset) * blend_factor
            ),
            .min_parallel_threshold = static_cast<size_t>(
                static_cast<double>(base_chars.min_parallel_threshold) * (1.0 - blend_factor) +
                static_cast<double>(base_chars.min_parallel_threshold + refinements.parallel_threshold_offset) * blend_factor
            ),
