...
class GoodnessOfFit {
   public:
    ...
    static bool performChiSquareTest(const DistributionType& distribution,
        ...
        auto [statistic, p_value, reject_null] =
            DistributionType::chiSquaredGoodnessOfFitTest(data, distribution, alpha);
    ...
    static bool performKolmogorovSmirnovTest(const DistributionType& distribution,
                                             const std::vector<double>& data, double alpha = 0.05) {
        auto [statistic, p_value, reject_null] =
            DistributionType::kolmogorovSmirnovTest(data, distribution, alpha);
        return !reject_null;  // Return true if we don't reject null (good fit)
    }
};
...
class CrossValidation {
   public:
    ...
    static double performKFoldValidation(const DistributionType& distribution,
                                         const std::vector<double>& data, int k = 5,
   ...
   public:
    ...
    static double calculateAIC(const DistributionType& distribution,
        ...
        auto [aic, bic, aicc, loglik] =
            DistributionType::computeInformationCriteria(data, distribution);
        return aic;
    }
...
class Bootstrap {
   public:
    ...
    static ConfidenceIntervalResult calculateParameterConfidenceIntervals(
        const DistributionType& distribution, const std::vector<double>& data,
        int num_bootstrap = 1000, double confidence_level = 0.95, unsigned int random_seed = 42) {
