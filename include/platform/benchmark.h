...
class Benchmark {
   ...
   public:
    ...
    explicit Benchmark(bool enableWarmup = true, std::size_t defaultIterations = 0,
    ...
    void addTest(const std::string& name, TestFunction testFunc, std::size_t iterations = 0,
                 double operationCount = 1.0, SetupFunction setupFunc = nullptr,
                 TeardownFunction teardownFunc = nullptr, std::size_t warmupRuns = 0);
   ...
   private:
    ...
    BenchmarkStats calculateStatsRobust(const std::vector<double>& times,
...
#define LIBSTATS_BENCHMARK_SIMPLE(name, func) benchmark.addTest(name, func)

#define LIBSTATS_BENCHMARK_WITH_SETUP(name, func, setup, teardown)                                 \
    benchmark.addTest(name, func, 0, 1.0, setup, teardown)

#define LIBSTATS_BENCHMARK_ITERATIONS(name, func, iters) benchmark.addTest(name, func, iters)
...
class StatsBenchmarkUtils {
   public:
    ...
    static std::vector<std::vector<double>> createTestVectors(std::size_t minSize,
                                                              std::size_t maxSize,
    ...
    static std::vector<std::vector<double>> createTestMatrices(std::size_t minRows,
                                                               std::size_t maxRows,
                                                               std::size_t minCols,
                                                               std::size_t maxCols,
    ...
    static void benchmarkBasicStats(const std::vector<std::vector<double>>& data,
                                    Benchmark& benchmark);

    /// Benchmark matrix operations
    /// @param matrices Test matrices
    /// @param benchmark Benchmark object to add tests to
    static void benchmarkMatrixOps(const std::vector<std::vector<double>>& matrices,
                                   Benchmark& benchmark);

    /// Benchmark SIMD vs scalar implementations
    /// @param data Test data
    /// @param benchmark Benchmark object to add tests to
    static void benchmarkSIMDOperations(const std::vector<std::vector<double>>& data,
                                        Benchmark& benchmark);
};
...
class RegressionTester {
   ...
   public:
    ...
    explicit RegressionTester(const std::string& baselineFile, double tolerancePercent = 5.0);
