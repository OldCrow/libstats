...
// Uses std::invoke_result_t when available (C++20), falls back to std::result_of for older compilers
...
public:
    ...
    static void parallelFor(std::size_t start, std::size_t end, Func&& task, 
        ...
        const std::size_t actualGrainSize = (grainSize == 0) ? 
            constants::parallel::adaptive::grain_size() : grainSize;
    ...
    static void parallelTransform(const T* input, T* output, std::size_t size,
                                 Func&& func, std::size_t grainSize = 0) {
        ...
        const std::size_t baseGrainSize = (grainSize == 0) ? 
            constants::parallel::adaptive::grain_size() : grainSize;
        
        // Align grain size to SIMD width for optimal performance
        const std::size_t alignedGrainSize = ((baseGrainSize + simdWidth - 1) / simdWidth) * simdWidth;
        
        parallelFor(0, size, [&](std::size_t i) {
    ...
    static T parallelSum(std::span<const T> data, std::size_t grainSize = 0) {
        ...
        return parallelReduce(0, size, T{0},
    ...
    static double parallelVariance(std::span<const T> data, 
        ...
        const double sumSquaredDiffs = parallelReduce(0, size, 0.0,
    ...
    static auto parallelStatOperation(std::span<const T> data, Op&& operation, Combiner&& combiner,
        ...
        for (std::size_t i = 0; i < size; i += alignedGrainSize) {
            ...
            auto future = pool.submit([&operation, chunk]() {
