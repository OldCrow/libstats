...
class DiscreteDistribution : public DistributionBase {
   public:
     ...
     * For Discrete Uniform distribution, mode is not unique - any value in {a, a+1, ..., b} is a
     * mode. This method returns the midpoint (a + b) / 2.0 as a representative mode.
     *
     ...
     *
     * For discrete uniform distribution: log(P(X = k)) = -log(b-a+1) for k ∈ {a, a+1, ..., b}, -∞
     * otherwise
     *
     ...
     * Efficiently fits discrete distribution parameters to multiple independent datasets in
     * parallel
     *
