...
public:
    static std::vector<std::vector<double>> generateGaussianDatasets(
    ...
    static std::vector<std::vector<double>> generateExponentialDatasets(
        size_t count, size_t size, std::mt19937& rng) {
    ...
    static std::vector<std::vector<double>> generateUniformDatasets(
        size_t count, size_t size, std::mt19937& rng) {
    ...
    static std::vector<std::vector<double>> generateGammaDatasets(
        size_t count, size_t size, std::mt19937& rng) {
    ...
    static std::vector<std::vector<double>> generatePoissonDatasets(
        size_t count, size_t size, std::mt19937& rng) {
    ...
    static std::vector<std::vector<double>> generateDiscreteDatasets(
        size_t count, size_t size, std::mt19937& rng) {
...
public:
    static FittingResult benchmarkParallelFitting(
        ...
        try {
            ...
            if (verbose) {
                std::cout << "    " << name << ": " 
                         ...
                         << result.parallel_time_ms << "ms parallel, "
                         << result.sequential_time_ms << "ms sequential, "
...
private:
    static double computeAccuracy(
        const std::vector<Distribution>& parallel_results,
    ...
    static double computeParameterError(const Distribution& a, const Distribution& b) {
        ...
        double var_error = std::abs(a.getVariance() - b.getVariance()) / (std::abs(b.getVariance()) + 1e-10);
...
private:
    template<typename Distribution>
    void runDistributionBenchmark(
        const std::string& name,
        std::vector<std::vector<double>> (*generator)(size_t, size_t, std::mt19937&),
        BenchmarkSummary& summary) {
        ...
        for (size_t dataset_count : config_.dataset_counts) {
            for (size_t dataset_size : config_.dataset_sizes) {
                std::cout << "  Testing " << dataset_count << " datasets of size " 
                         << dataset_size << "...\n";
                ...
                if (!result.success) {
                    ...
                    std::cout << "    ✅ Success: " 
                             << std::fixed << std::setprecision(2)
                             << result.speedup << "x speedup, "
                             << (result.efficiency * 100.0) << "% efficiency\n";
                }
            }
        }
    }
    ...
    void printSummary(const BenchmarkSummary& summary) {
        ...
        std::cout << std::left << std::setw(12) << "Distribution"
                 << std::right << std::setw(10) << "Avg Speedup"
                 << std::setw(12) << "Max Speedup" 
        ...
        for (const auto& [dist_name, results] : by_distribution) {
            ...
            std::cout << std::left << std::setw(12) << dist_name
                     ...
                     << std::setw(10) << avg_speedup << "x"
                     << std::setw(11) << max_speedup << "x"
                     << std::setw(10) << (avg_efficiency * 100.0) << "%"
    ...
    void analyzeScalability(const BenchmarkSummary& summary) {
        ...
        for (const auto& [size, results] : by_dataset_size) {
            ...
            for (const auto* result : sorted_results) {
                std::cout << "    " << std::setw(3) << result->dataset_count 
                         << " datasets → " << std::fixed << std::setprecision(2)
                         << result->speedup << "x speedup\n";
            }
        }
...
BenchmarkConfig parseArguments(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--quick") {
            ...
            std::cout << "                       (gaussian, exponential, uniform, gamma, poisson, discrete)\n";
