...
inline void recordPerformanceMicroseconds(libstats::performance::Strategy         strategy,
                                          libstats::performance::DistributionType dist_type, size_t data_size,
                                          double time_microseconds)
{
    constexpr double MICROSECONDS_TO_NANOSECONDS = 1000.0;
    libstats::performance::PerformanceDispatcher::recordPerformance(
        strategy, dist_type, data_size, static_cast<uint64_t>(time_microseconds * MICROSECONDS_TO_NANOSECONDS));
}
} // namespace perf_utils
...
inline void displaySystemCapabilities()
{
    ...
    std::cout << std::setw(25) << "Memory Bandwidth:" << std::setprecision(2) << capabilities.memory_bandwidth_gb_s()
              << " GB/s\n";
    std::cout << std::setw(25) << "Thread Overhead:" << std::setprecision(1) << capabilities.threading_overhead_ns()
              << " ns\n";
...
inline void displayCompactSystemInfo()
{
    ...
    std::cout << "System: " << capabilities.logical_cores() << " logical cores, " << simd_level << " SIMD, "
...
inline void displayAdaptiveConstants()
{
    ...
    std::cout << formatter.formatRow(
                     {"Min Elements for Parallel",
                      std::to_string(libstats::constants::parallel::adaptive::min_elements_for_parallel())})
              << "\n";
    std::cout << formatter.formatRow(
                     {"Default Grain Size", std::to_string(libstats::constants::parallel::adaptive::grain_size())})
              << "\n";
    std::cout << formatter.formatRow(
                     {"Simple Operation Grain Size",
                      std::to_string(libstats::constants::parallel::adaptive::simple_operation_grain_size())})
              << "\n";
    std::cout << formatter.formatRow(
                     {"Complex Operation Grain Size",
                      std::to_string(libstats::constants::parallel::adaptive::complex_operation_grain_size())})
              << "\n";
...
inline void displayPlatformConstants()
{
    ...
    std::cout << formatter.formatRow(
                     {"SIMD Block Size", std::to_string(platform::get_optimal_simd_block_size()) + " doubles"})
              << "\n";
    ...
    std::cout << formatter.formatRow({"Min SIMD Size", std::to_string(platform::get_min_simd_size()) + " elements"})
              << "\n";
    std::cout << formatter.formatRow(
                     {"Optimal Grain Size", std::to_string(platform::get_optimal_grain_size()) + " elements"})
              << "\n";
    std::cout << formatter.formatRow(
                     {"Fast Transcendental Support", platform::supports_fast_transcendental() ? "Yes" : "No"})
              << "\n";
 ...
 */
template <typename Func> inline int runTool(const std::string &tool_name, Func &&func)
{
    try
    {
 ...
 */
inline void printBenchmarkResults(const std::string &operation_name, double serial_time, double parallel_time,
