...
private:
    ...
    bool compare_results(const std::vector<double>& reference,
                        const std::vector<double>& test,
                        double& max_diff, size_t& failed_count,
                        std::string& error_details) {
        max_diff = 0.0;
        failed_count = 0;
        error_details.clear();
        ...
        if (reference.size() != test.size()) {
            error_details = "Size mismatch: " + std::to_string(reference.size()) + " vs " + std::to_string(test.size());
            return false;
        }
    ...
    template<typename Dist>
    void test_threading_system(const std::string& dist_name, 
                              const std::string& system_name,
                              std::function<std::vector<double>(const std::vector<double>&, const std::string&)> compute_func) {
        ...
        for (const std::string& operation : {"PDF", "LogPDF", "CDF"}) {
            ...
            try {
                ...
                size_t failed_count;
                std::string error_details;
                result.correctness_pass = compare_results(seq_results, par_results, max_diff, failed_count, error_details);
                ...
                if (config_.test_determinism) {
                    for (size_t run = 1; run < config_.num_runs; ++run) {
                        ...
                        bool deterministic = compare_results(par_results, run_results, run_max_diff, run_failed_count, run_error_details);
                ...
                if (config_.test_thread_safety) {
                    ...
                    for (int i = 0; i < concurrent_tests; ++i) {
                        futures.push_back(std::async(std::launch::async, compute_func, test_data, operation));
                    }
                    ...
                    for (auto& future : futures) {
                        ...
                        bool thread_safe = compare_results(par_results, concurrent_results, concurrent_max_diff, concurrent_failed_count, concurrent_error_details);
                        
                        if (!thread_safe) {
                            result.thread_safety_pass = false;
                            if (result.error_details.find("Thread safety") == std::string::npos) {
                                result.error_details += " Thread safety issue: " + concurrent_error_details;
                            }
                        }
                    }
                }
            ...
            // Display result
            std::string status = (result.correctness_pass && result.determinism_pass && result.thread_safety_pass) ? "✓ PASS" : "✗ FAIL";
            std::cout << std::setw(8) << operation << " [" << std::setw(12) << system_name << "]: " << status;
            std::cout << " (max_diff=" << std::scientific << std::setprecision(2) << result.max_difference;
            std::cout << ", speedup=" << std::fixed << std::setprecision(1) << result.performance_ratio << "x";
    ...
    void test_distribution(const std::string& dist_name) {
        ...
        test_threading_system<Dist>(dist_name, "ThreadPool", 
        ...
        test_threading_system<Dist>(dist_name, "GCD", 
            [this](const std::vector<double>& inputs, const std::string& operation) {
...
#if defined(LIBSTATS_HAS_OPENMP)
        // Test OpenMP
        test_threading_system<Dist>(dist_name, "OpenMP", 
            [this](const std::vector<double>& inputs, const std::string& operation) {
    ...
    void print_summary() {
        ...
        for (const auto& [system, system_results] : by_system) {
            ...
            for (const auto& result : system_results) {
                if (result->correctness_pass) correctness_passed++;
                if (result->determinism_pass) determinism_passed++;
                if (result->thread_safety_pass) thread_safety_passed++;
        ...
        for (auto& result : results_) {
            if (!result.correctness_pass || !result.determinism_pass || !result.thread_safety_pass) {
        ...
        std::cout << "\n--- Recommendations ---\n";
        if (failed_tests.empty()) {
            std::cout << "✅ All parallel implementations are correct, deterministic, and thread-safe.\n";
        } else {
        ...
        if (overall_avg_speedup > 1.5) {
            ...
            std::cout << "⚠️  Parallel speedup is lower than expected (avg " << std::fixed << std::setprecision(1) << overall_avg_speedup << "x).\n";
