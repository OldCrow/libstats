...
RecoveryStats compute_stats(const std::vector<double>& means, const std::vector<double>& vars,
    ...
    if (s.n_ok == 0) return s;

    double sum_mean_err = 0, sum_mean_sq = 0;
    double sum_var_err  = 0, sum_var_sq  = 0;
    for (int i = 0; i < s.n_ok; ++i) {
        double me = means[i] - true_mean;
        double ve = vars[i]  - true_var;
        sum_mean_err += me;  sum_mean_sq += me * me;
        sum_var_err  += ve;  sum_var_sq  += ve * ve;
    }
...
int main(int argc, char* argv[]) {
    ...
    return stats::detail::detail::runTool("Parameter Recovery Benchmark", [quick_mode]() {
        ...
        const std::vector<size_t> sample_sizes = quick_mode
            ? std::vector<size_t>{50, 100, 500}
        ...
        std::string header = fmt.formatRow({"n", "ok/M",
            "MeanBias", "MeanRMSE", "VarBias", "VarRMSE"});
