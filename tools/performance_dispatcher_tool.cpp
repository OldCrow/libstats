...
class PerformanceDispatcherTool {
   ...
   private:
    ...
    void simulatePerformanceData(PerformanceHistory& history) {
        // Simulate realistic performance patterns using the same modeling as
        // threshold_learning_demo
        ...
        std::map<DistributionType, std::pair<double, double>> efficiency_characteristics = {
            {DistributionType::UNIFORM,
             {0.40, 0.25}},  // Good SIMD/Parallel efficiency - simple ops
        ...
        for (auto dist_type : distributions) {
            ...
            for (auto size : sizes) {
                // Record multiple samples per strategy to reach the reliable data threshold (>=5
                // samples)
                for (int sample = 0; sample < SAMPLES_PER_STRATEGY / 4;
                     ++sample) {  // Use fewer samples per size for broader coverage
                    // Scalar strategy - affected by computational complexity
                    auto scalar_time = static_cast<uint64_t>(
                        static_cast<double>(size) * timing_simulation::SCALAR_PERFORMANCE_FACTOR *
                        complexity * noise(rng_));
                    ...
                    auto simd_time = static_cast<uint64_t>(
                        static_cast<double>(size) * timing_simulation::SIMD_PERFORMANCE_FACTOR *
                    ...
                    auto parallel_time = static_cast<uint64_t>(
                        static_cast<double>(size) * timing_simulation::PARALLEL_PERFORMANCE_FACTOR *
                        complexity * parallel_efficiency * noise(rng_));

                    // More realistic parallel overhead model - decreases with complexity and size
                    double complexity_factor = complexity;
                    double overhead_reduction = std::max(
                        1.0, static_cast<double>(size) / 1000.0);  // Overhead reduces with size
