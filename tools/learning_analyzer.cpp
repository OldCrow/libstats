 ...
 * @brief Consolidated learning analysis tool combining real execution analysis and educational simulation
 * 
 * This tool consolidates the functionality of adaptive_learning_analyzer.cpp and threshold_learning_demo.cpp,
 * providing both comprehensive performance analysis with real execution data and educational simulation
...
public:
    ...
    void runAnalysis() {
        ...
        std::vector<size_t> batch_sizes = {
            5, 8, 10, 16, 20, 25, 32, 40, 50, 64, 80, 100, 
            128, 160, 200, 256, 320, 400, 500, 640, 800, 1000,
            1280, 1600, 2000, 2560, 3200, 4000, 5000, 6400, 8000, 10000,
            12800, 16000, 20000, 25600, 32000, 40000, 50000, 64000, 80000, 100000
        };
        
        std::cout << "Testing " << batch_sizes.size() << " different batch sizes across all distributions...\n\n";
...
private:
    void showInitialState() {
        ...
        for (auto size : test_sizes) {
            ...
            auto gaussian_strategy = dispatcher.selectOptimalStrategy(
                ...
                ComputationComplexity::MODERATE, 
                capabilities
            );
                
            std::cout << std::setw(12) << size 
                      << std::setw(20) << strings::strategyToDisplayString(uniform_strategy)
                      << std::setw(20) << strings::strategyToDisplayString(gaussian_strategy) << "\n";
        }
        std::cout << "\n";
    }
    ...
    void simulatePerformanceLearning() {
        ...
        std::cout << "Recording performance data across different distributions and batch sizes...\n";
        ...
        for (auto dist_type : distributions) {
            std::cout << "\n  Simulating " << strings::distributionTypeToString(dist_type) << " distribution:\n";
            ...
            for (auto size : sizes) {
                ...
                for (int sample = 0; sample < SAMPLES_PER_STRATEGY; ++sample) {
                    // Scalar strategy
                    auto scalar_time = static_cast<uint64_t>(static_cast<double>(size) * SCALAR_PERFORMANCE_FACTOR * complexity * noise(rng_));
                    history.recordPerformance(Strategy::SCALAR, dist_type, size, scalar_time);
                    
                    // SIMD strategy
                    auto simd_time = static_cast<uint64_t>(static_cast<double>(size) * SIMD_PERFORMANCE_FACTOR * complexity * simd_efficiency * noise(rng_));
                    ...
                    // Parallel strategy
                    auto parallel_time = static_cast<uint64_t>(static_cast<double>(size) * PARALLEL_PERFORMANCE_FACTOR * complexity * parallel_efficiency * noise(rng_));
                    double complexity_factor = complexity;
                    double overhead_reduction = std::max(1.0, static_cast<double>(size) / 1000.0);
                    uint64_t base_overhead = static_cast<uint64_t>(8000.0 / complexity_factor / overhead_reduction);
    ...
    void showLearnedStrategies() {
        ...
        for (auto size : test_sizes) {
            ...
            std::cout << std::setw(12) << size 
                      << std::setw(20) << strings::strategyToDisplayString(recommendation.recommended_strategy)
                      << std::setw(15) << format::confidenceToString(recommendation.confidence_score)
                      << std::setw(12) << format::nanosecondsToMicroseconds(recommendation.expected_time_ns) << "\n";
        }
        std::cout << "\n";
    }
    ...
    void demonstrateAdaptiveSelection() {
        ...
        for (auto dist_type : {DistributionType::UNIFORM, DistributionType::GAUSSIAN,
            ...
            if (thresholds.has_value()) {
                ...
                std::cout << "  " << strings::distributionTypeToString(dist_type) << ": Insufficient data\n";
            }
        }
    ...
    void analyzePerformanceHistory() {
        ...
        for (auto dist_type : distributions) {
            std::cout << "\n" << strings::distributionTypeToString(dist_type) << " Distribution:" << std::endl;
            ...
            for (size_t size : test_sizes) {
                ...
                std::cout << "  " << std::setw(8) << size
                         << "  " << std::setw(14) << strings::strategyToDisplayString(recommendation.recommended_strategy)
                         << "  " << std::setw(10) << format::confidenceToString(recommendation.confidence_score)
                         << "  " << std::setw(8) << format::nanosecondsToMicroseconds(recommendation.expected_time_ns)
        ...
        for (auto dist_type : distributions) {
            std::cout << "\n" << strings::distributionTypeToString(dist_type) << " Performance:" << std::endl;
            ...
            for (auto strategy : strategies) {
                ...
                if (stats) {
                    std::cout << "  " << std::setw(14) << strings::strategyToDisplayString(strategy) << ": "
                             ...
                             << "avg: " << std::setw(8) << format::nanosecondsToMicroseconds(stats->getAverageTimeNs()) << ", "
                             << "min: " << std::setw(6) << format::nanosecondsToMicroseconds(stats->min_time_ns) << ", "
                             << "max: " << std::setw(6) << format::nanosecondsToMicroseconds(stats->max_time_ns)
                             << std::endl;
                }
            }
        }
    ...
    template<typename Distribution>
    void exerciseDistributionEnhanced(const std::string& /* dist_name */, DistributionType dist_type, 
                                     Distribution& dist, const std::vector<size_t>& batch_sizes) {
        std::random_device rd;
        std::mt19937 gen(rd());
    ...
    void generatePerformanceInsights(PerformanceHistory& history, 
        ...
        for (auto strategy : {Strategy::SIMD_BATCH, Strategy::PARALLEL_SIMD, 
            ...
            for (auto dist_type : distributions) {
                ...
                if (scalar_stats && strategy_stats && 
                    scalar_stats->execution_count > 0 && strategy_stats->execution_count > 0) {
                    total_distributions++;
