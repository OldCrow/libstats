...
- 📦 **Header Optimization**: Complete PIMPL conversion for remaining 38 files
...
| **v0.9.2** | 2-3 weeks | Distribution Code Refactoring | Medium |
| **v0.9.3** | 1-2 weeks | Header Optimization Completion | Low |
| **v0.9.4** | 1 week | Final Polish & Documentation | Low |
...
## 🏗️ v0.9.2 - Distribution Code Refactoring (2-3 weeks)

**Goal**: Eliminate ~8,000 lines of duplicated code across distributions using template-based CRTP patterns
...
### 🔧 **Task 8: Complete Phase 2 PIMPL Header Optimization**

**Status**: Infrastructure complete, 38 files need conversion
**Expected Impact**: 15-25% compilation improvement + template hiding benefit

#### Checklist:
- [ ] **Convert files using `platform/platform_constants.h` to forward declarations (15 files)**
  ...
  - [ ] `src/adaptive_cache.cpp` → use forward declarations
  ...
  - [ ] **Test files**: Evaluate if `tests/test_constants.cpp` needs full header for testing

- [ ] **Convert files using `platform/parallel_execution.h` to forward declarations (10 files)**
  - [ ] `tools/parallel_correctness_verification.cpp` → use `platform/parallel_execution_fwd.h`
  - [ ] `src/distribution_base.cpp` → use forward declarations
  ...
  - [ ] **Evaluate special cases**: 
    - [ ] `tests/test_parallel_compilation.cpp` (may need full header)
    - [ ] `include/libstats.h` (main header - may need full header for API completeness)

- [ ] **STL consolidation header conversion (97+ files)**
  - [ ] **Vector consolidation**: Replace standalone `#include <vector>` with `#include "common/libstats_vector_common.h"` (47+ files)
  - [ ] **String consolidation**: Replace standalone `#include <string>` with `#include "common/libstats_string_common.h"` (32+ files)  
  - [ ] **Algorithm consolidation**: Replace standalone `#include <algorithm>` with `#include "common/libstats_algorithm_common.h"` (18+ files)

- [ ] **Measure compilation improvements**
  - [ ] Baseline compilation times before conversion
  - [ ] Measure incremental improvements after each batch of conversions
  - [ ] Target 15-25% overall improvement
  - [ ] Document template hiding benefits from PIMPL integration
