...
std::pair<double, double> clopperPearsonCI(int k, int n, double confidence_level) {
    if (n < 1)
    ...
    const double lower = (k == 0) ? 0.0
                                  : detail::inverse_beta_i(alpha / 2.0, static_cast<double>(k),
                                                           static_cast<double>(n - k + 1));

    // Upper bound: Beta(1-α/2, k+1, n-k) — 1 when k=n
    const double upper = (k == n)
                             ? 1.0
                             : detail::inverse_beta_i(1.0 - alpha / 2.0, static_cast<double>(k + 1),
                                                      static_cast<double>(n - k));

    return {lower, upper};
}

std::tuple<double, double, bool> proportionZTest(int k, int n, double p0, double alpha) {
    if (n < 1)
        throw std::invalid_argument("n must be at least 1");
    if (k < 0 || k > n)
        throw std::invalid_argument("k must satisfy 0 <= k <= n");
...
std::tuple<double, double, bool> twoProportionZTest(int k1, int n1, int k2, int n2, double alpha) {
    if (n1 < 1 || n2 < 1)
