...
LaplaceDistribution::LaplaceDistribution(double mu, double b) : DistributionBase(), mu_(mu), b_(b) {
...
LaplaceDistribution::LaplaceDistribution(double mu, double b, bool /*bypassValidation*/) noexcept
...
void LaplaceDistribution::fit(const std::vector<double>& values) {
    ...
    if (!std::isfinite(b_hat) || b_hat <= detail::ZERO_DOUBLE)
        throw std::invalid_argument(
            "Laplace MLE produced a degenerate scale estimate (b ≤ 0); data may be constant or "
...
void LaplaceDistribution::getProbability(std::span<const double> values, std::span<double> results,
...
std::istream& operator>>(std::istream& is, LaplaceDistribution& dist) {
    ...
    if (mu_pos == std::string::npos || b_pos == std::string::npos || close == std::string::npos) {
        is.setstate(std::ios::failbit);
        return is;
    }
...
void LaplaceDistribution::getLogProbabilityBatchUnsafeImpl(const double* values, double* results,
                                                           std::size_t count, double cached_mu,
                                                           double cached_neg_inv_b,
...
void LaplaceDistribution::getProbabilityBatchUnsafeImpl(const double* values, double* results,
                                                        std::size_t count, double cached_mu,
                                                        double cached_neg_inv_b,
                                                        double cached_neg_log2b) const noexcept {
    using VectorOps = arch::simd::VectorOps;
    ...
    getLogProbabilityBatchUnsafeImpl(values, results, count, cached_mu, cached_neg_inv_b,
                                     cached_neg_log2b);
