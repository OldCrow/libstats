...
Benchmark::Benchmark(bool enableWarmup, std::size_t defaultIterations,
                     std::size_t defaultWarmupRuns)
...
void Benchmark::addTest(const std::string& name, TestFunction testFunc, std::size_t iterations,
                        double operationCount, SetupFunction setupFunc,
                        TeardownFunction teardownFunc, std::size_t warmupRuns) {
    TestCase testCase;
    testCase.name = name;
...
void Benchmark::compareResults(const std::vector<BenchmarkResult>& baseline,
                               const std::vector<BenchmarkResult>& comparison, std::ostream& os) {
    os << "\n" << std::string(80, '=') << "\n";
    ...
    for (const auto& [name, baseResult] : baselineMap) {
        ...
        if (it != comparisonMap.end()) {
            ...
            os << "  Baseline:   " << std::fixed << std::setprecision(6) << baseResult->stats.mean
               << "s\n";
            os << "  Comparison: " << std::fixed << std::setprecision(6) << compResult.stats.mean
               << "s\n";
...
BenchmarkStats Benchmark::calculateStats(const std::vector<double>& times,
                                         double operationCount) const {
    if (times.empty()) {
        return {};
    }
...
std::vector<std::vector<double>> StatsBenchmarkUtils::createTestVectors(std::size_t minSize,
                                                                        std::size_t maxSize,
...
std::vector<std::vector<double>> StatsBenchmarkUtils::createTestMatrices(std::size_t minRows,
                                                                         std::size_t maxRows,
                                                                         std::size_t minCols,
                                                                         std::size_t maxCols,
...
void StatsBenchmarkUtils::benchmarkBasicStats(const std::vector<std::vector<double>>& data,
                                              Benchmark& benchmark) {
    if (data.empty())
        return;
...
void StatsBenchmarkUtils::benchmarkMatrixOps(const std::vector<std::vector<double>>& matrices,
                                             Benchmark& benchmark) {
    if (matrices.empty())
        return;
...
void StatsBenchmarkUtils::benchmarkSIMDOperations(const std::vector<std::vector<double>>& data,
                                                  Benchmark& benchmark) {
    if (data.empty())
        return;
...
RegressionTester::RegressionTester(const std::string& baselineFile, double tolerancePercent)
    : baselineFile_(baselineFile), tolerancePercent_(tolerancePercent) {}
...
bool RegressionTester::saveBaseline(const std::vector<BenchmarkResult>& results) {
    ...
    for (const auto& result : results) {
        file << result.name << "," << std::fixed << std::setprecision(9) << result.stats.mean << ","
             << result.stats.median << "," << result.stats.stddev << "," << result.stats.min << ","
             << result.stats.max << "," << result.stats.samples << "," << result.stats.throughput
             << "\n";
    }
...
bool RegressionTester::checkRegressions(const std::vector<BenchmarkResult>& currentResults,
    ...
    for (const auto& current : currentResults) {
        ...
        os << current.name << " (" << std::showpos << std::fixed << std::setprecision(1)
           << percentChange << "%"
...
BenchmarkStats Benchmark::calculateStatsRobust(const std::vector<double>& times,
                                               double operationCount) const {
    if (times.empty()) {
        return {};
    }
