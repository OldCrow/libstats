...
// Use compiler-specific approaches to disable SIMD in CPU detection code
    ...
    #define CPU_DETECTION_NO_SIMD __attribute__((target("no-avx512f,no-avx512cd,no-avx512bw,no-avx512dq,no-avx512vl,no-avx2,no-avx,no-sse4.2,no-sse4.1,no-ssse3,no-sse3")))
#elif defined(__GNUC__)
    // GCC: use pragma to disable SIMD globally for this file
    #pragma GCC push_options
    #pragma GCC target("no-avx512f,no-avx512cd,no-avx512bw,no-avx512dq,no-avx512vl,no-avx2,no-avx,no-sse4.2,no-sse4.1,no-ssse3,no-sse3")
...
namespace {
     ...
     */
    void safe_cpuid(uint32_t eax, uint32_t ecx, uint32_t& out_eax, uint32_t& out_ebx, 
    ...
    void detect_performance_info(Features& features) {
        ...
        if (features.performance.has_rdtsc) {
            features.performance.tsc_frequency = estimate_tsc_frequency_internal(constants::simd::cpu::DEFAULT_TSC_SAMPLE_MS); // Quick sample
        } else {
...
std::string features_string() {
    ...
    if (f.sse2) add_feature("SSE2");
    if (f.sse3) add_feature("SSE3");
    if (f.ssse3) add_feature("SSSE3");
    if (f.sse4_1) add_feature("SSE4.1");
    if (f.sse4_2) add_feature("SSE4.2");
    if (f.avx) add_feature("AVX");
    if (f.fma) add_feature("FMA");
    if (f.avx2) add_feature("AVX2");
    if (f.avx512f) add_feature("AVX512F");
    if (f.neon) add_feature("NEON");
    if (f.sve) add_feature("SVE");
...
size_t optimal_double_width() {
    ...
    if (f.avx512f) return 8;  // 512 bits / 64 bits per double
    if (f.avx || f.avx2) return 4;  // 256 bits / 64 bits per double
    if (f.sse2) return 2;  // 128 bits / 64 bits per double
    if (f.neon) return 2;  // 128 bits / 64 bits per double
    return 1;  // Scalar
}
...
size_t optimal_float_width() {
    ...
    if (f.avx512f) return 16;  // 512 bits / 32 bits per float
    if (f.avx || f.avx2) return 8;   // 256 bits / 32 bits per float
    if (f.sse2) return 4;   // 128 bits / 32 bits per float
    if (f.neon) return 4;   // 128 bits / 32 bits per float
    return 1;  // Scalar
}
...
size_t optimal_alignment() {
    ...
    if (f.avx512f) return 64;  // 512 bits = 64 bytes
    if (f.avx || f.avx2) return 32;  // 256 bits = 32 bytes
    if (f.sse2 || f.neon) return 16;  // 128 bits = 16 bytes
...
std::string detailed_cpu_info() {
    ...
    result += "  Hyperthreading: " + (f.topology.hyperthreading ? std::string("Yes") : std::string("No")) + "\n";
...
bool validate_feature_consistency() {
    ...
    if (f.avx2 && !f.avx) return false;
    if (f.avx && !f.sse2) return false;
    if (f.avx512f && !f.avx2) return false;
    
    // Check SSE hierarchy consistency
    if (f.sse4_2 && !f.sse4_1) return false;
    if (f.sse4_1 && !f.ssse3) return false;
    if (f.ssse3 && !f.sse3) return false;
    if (f.sse3 && !f.sse2) return false;
    
    // Check AVX-512 sub-features require AVX-512F
    if ((f.avx512dq || f.avx512cd || f.avx512bw || f.avx512vl) && !f.avx512f) return false;
