...
void VectorOps::vector_erf_avx512(const double* values, double* results,
    ...
    for (std::size_t i = 0; i < simd_end; i += W) {
        ...
        __m512d RS = _mm512_div_pd(_mm512_mask_blend_pd(m3, R4, R3),
                                   _mm512_mask_blend_pd(m3, S4, S3));

        // exp_arg = -x²-0.5625+R/S; clamp to ≤ 0 (erfc ≤ 1 for |x| ≥ 1.25)
        __m512d exp_arg = _mm512_sub_pd(_mm512_sub_pd(RS, c0p5625),
                                        _mm512_mul_pd(sax, sax));
        exp_arg = _mm512_min_pd(exp_arg, _mm512_setzero_pd());
        _mm512_store_pd(exp_buf, exp_arg);
        vector_exp_avx512(exp_buf, exp_buf, W);
        __m512d r34 = _mm512_sub_pd(one,
                                    _mm512_div_pd(_mm512_load_pd(exp_buf), sax));
    ...
    for (std::size_t i = simd_end; i < size; ++i) results[i] = std::erf(values[i]);
}
