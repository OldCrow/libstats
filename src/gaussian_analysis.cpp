...
std::tuple<double, double, bool> shapiroWilkTest(const std::vector<double>& data, double alpha) {
    ...
    for (std::size_t i = 0; i < n / 2; ++i) {
        ...
        const double p_i = (static_cast<double>(i) + 1.0 - 0.375) / (static_cast<double>(n) + 0.25);
...
std::tuple<double, double, bool> jarqueBeraTest(const std::vector<double>& data, double alpha) {
    if (data.size() < 8)
    ...
    // Jarque-Bera (1980): JB = n·(S²/6 + K²/24) where S = skewness, K = excess kurtosis.
    // The denominator for the kurtosis term is 24, not 225 (NEW-MC-1).
    const double jb =
...
std::pair<double, double> confidenceIntervalMean(const std::vector<double>& data,
    ...
    if (population_variance_known) {
        ...
        if (n >= 30) {
            ...
            const double t = detail::inverse_t_cdf(1.0 - alpha * 0.5, static_cast<double>(n - 1));
            margin = t * std_err;
        }
    }
...
std::pair<double, double> confidenceIntervalVariance(const std::vector<double>& data,
                                                     double confidence_level) {
    if (data.size() < 2)
...
std::tuple<double, double, bool> oneSampleTTest(const std::vector<double>& data,
                                                double hypothesized_mean, double alpha) {
    if (data.empty())
        throw std::invalid_argument("Data vector cannot be empty");
    if (alpha <= 0.0 || alpha >= 1.0)
        throw std::invalid_argument("Alpha must be between 0 and 1");
...
std::tuple<double, double, bool> twoSampleTTest(const std::vector<double>& data1,
                                                const std::vector<double>& data2,
                                                bool equal_variances, double alpha) {
    ...
    const double v2 =
        std::inner_product(data2.begin(), data2.end(), data2.begin(), 0.0, std::plus<>(),
                           [m2](double x, double y) { return (x - m2) * (y - m2); }) /
...
std::tuple<double, double, bool> pairedTTest(const std::vector<double>& data1,
                                             const std::vector<double>& data2, double alpha) {
...
std::tuple<double, double, double, double> bayesianEstimation(const std::vector<double>& data,
                                                              double prior_mean,
...
std::pair<double, double> bayesianCredibleInterval(const std::vector<double>& data,
                                                   double credibility_level, double prior_mean,
                                                   double prior_precision, double prior_shape,
                                                   double prior_rate) {
    auto [pm, pp, ps, pr] =
...
static double compute_robust_weight(double sr, const std::string& type, double c) {
    ...
    if (type == "hampel") {
        ...
        if (asr <= 2.0 * c)
            return c / asr;
        if (asr <= 3.0 * c)
            return c * (3.0 - asr / c) / (2.0 * asr);
        return 0.0;
    }
    throw std::invalid_argument("Unknown estimator type. Use 'huber', 'tukey', or 'hampel'");
}

std::pair<double, double> robustEstimation(const std::vector<double>& data,
    ...
    const double mad = (n % 2 == 0) ? (abs_dev[n / 2 - 1] + abs_dev[n / 2]) / 2.0 : abs_dev[n / 2];
    ...
    for (int iter = 0; iter < 50; ++iter) {
        ...
        for (double x : data) {
            const double w =
                compute_robust_weight((x - loc) / scale, estimator_type, tuning_constant);
            sum_w += w;
            sum_wx += w * x;
        }
...
std::pair<double, double> methodOfMomentsEstimation(const std::vector<double>& data) {
    if (data.empty())
        throw std::invalid_argument("Data vector cannot be empty");
    const std::size_t n = data.size();
    const double mean = std::accumulate(data.begin(), data.end(), 0.0) / n;
...
std::pair<double, double> lMomentsEstimation(const std::vector<double>& data) {
    if (data.size() < 2)
    ...
    for (std::size_t i = 0; i < n; ++i) {
        const double w =
            (2.0 * static_cast<double>(i) + 1.0 - static_cast<double>(n)) / static_cast<double>(n);
        l2 += w * sorted[i];
    }
...
std::vector<double> calculateHigherMoments(const std::vector<double>& data, bool center_on_mean) {
    if (data.empty())
        throw std::invalid_argument("Data vector cannot be empty");
