...
- Target-first scoping, `LIBSTATS_`-prefixed options, warnings PRIVATE and
  `PROJECT_IS_TOP_LEVEL`-gated: landed (Phase 3B). Where the threading and
  compiler-flag logic lives, and how warnings are applied per target:
  `docs/BUILD_SYSTEM_GUIDE.md`. Optimization/debug-info flags for the custom
  `Dev`/`Strict` build types use the guarded-FORCE idiom
...
- **A configure-time fact that a public header branches on goes in the
  ...
  The #97 export incident that prompted it: `docs/BUILD_SYSTEM_GUIDE.md`.
