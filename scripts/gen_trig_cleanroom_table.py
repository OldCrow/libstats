#!/usr/bin/env python3
"""Generate src/trig_cleanroom_data.inc -- shared constants for the x86
vector_cos_<tier>/vector_sin_<tier> clean-room quadrant-reduction kernels
(issue #95).

Ported from libhmm's scripts/gen_trig_cleanroom_table.py (same owner, MIT,
clean-room derived, no third-party source), itself a port of this project's
own scripts/gen_neon_trig_cleanroom_table.py. The math is byte-for-byte
identical to that generator -- same precision, same splits, same fits -- so
the emitted kTrig* constants are REQUIRED to be bit-identical to the
kTrigNeon* constants already checked in at src/neon_trig_cleanroom_data.inc;
this script self-checks that identity and exits non-zero on any mismatch
...
Usage:  <path-to-python-with-mpmath> scripts/gen_trig_cleanroom_table.py
Writes src/trig_cleanroom_data.inc relative to the repo root, then
self-checks it bit-for-bit against src/neon_trig_cleanroom_data.inc.
"""
...
out = os.path.join(root, "src", "trig_cleanroom_data.inc")
with open(out, "w") as f:
    f.write("// Auto-generated by scripts/gen_trig_cleanroom_table.py -- do not edit.\n")
    f.write("// Clean-room derived constants for the x86 vector_cos_<tier>/\n")
    f.write("// vector_sin_<tier> kernels (issue #95); bit-identical to the\n")
    f.write("// kTrigNeon* constants in src/neon_trig_cleanroom_data.inc (verified by\n")
    f.write("// this script's self-check). See docs/NEON_TRIG_DERIVATION.md and\n")
    f.write("// docs/NEON_TRIG_DIVERGENCE_AUDIT.md for the mathematics and the\n")
    f.write("// point-by-point comparison against ARM optimized-routines advsimd\n")
    f.write("// sin.c/cos.c confirming no shared expression. No third-party source.\n\n")
    f.write(f"static constexpr double kTrigDMax = {as_hex(D_MAX)};  // 2^23\n")
    f.write(f"static constexpr double kTrigTwoOverPi = {as_hex(TWO_OVER_PI)};\n\n")
    ...
    f.write(f"static constexpr double kTrigPio2[{N_PARTS}] = {{\n")
    for p in PARTS:
        f.write(f"    {as_hex(p)},\n")
    f.write("};\n\n")
    ...
    f.write(f"static constexpr double kTrigSinC[{DEG + 1}] = {{\n")
    for c in SIN_C:
        f.write(f"    {as_hex(c)},\n")
    f.write("};\n")
    f.write(f"static constexpr double kTrigCosC[{DEG + 1}] = {{\n")
    for c in COS_C:
        f.write(f"    {as_hex(c)},\n")
    f.write("};\n")
...
# --- self-check: bit-identical to src/neon_trig_cleanroom_data.inc --------
def parse_neon_inc(path):
    """Pull every hex-float double literal out of the NEON .inc, keyed by the
    kTrigNeon* name it follows (scalars as a 1-element list, arrays as N)."""
    with open(path) as fh:
        text = fh.read()
    values = {}
    # Matches "kTrigNeonFoo = 0x...p+0;" or "kTrigNeonFoo[4] = { 0x..., ... };"
    for m in re.finditer(
        r"kTrigNeon(\w+?)(?:\[\d+\])?\s*=\s*(.*?);", text, re.DOTALL
    ):
        name, body = m.group(1), m.group(2)
        lits = re.findall(r"-?0x[0-9a-fA-F.]+p[+-]?\d+", body)
        values[name] = [float.fromhex(lit) for lit in lits]
    return values


neon_path = os.path.join(root, "src", "neon_trig_cleanroom_data.inc")
neon_values = parse_neon_inc(neon_path)
...
mismatches = []
for key, gen_vals in generated.items():
    neon_vals = neon_values.get(key)
    if neon_vals is None:
        mismatches.append(f"{key}: not found in {neon_path}")
        continue
    if len(neon_vals) != len(gen_vals):
        mismatches.append(
            f"{key}: length mismatch (generated {len(gen_vals)}, NEON {len(neon_vals)})"
        )
        continue
    for i, (g, n) in enumerate(zip(gen_vals, neon_vals)):
        # Bit-identical comparison via the hex representation (handles -0.0 too).
        if g.hex() != n.hex():
            mismatches.append(
                f"{key}[{i}]: generated {g.hex()} != NEON {n.hex()}"
            )

if mismatches:
    print("SELF-CHECK FAILED: generated kTrig* constants are not bit-identical "
          "to kTrigNeon* in src/neon_trig_cleanroom_data.inc:", file=sys.stderr)
    for msg in mismatches:
        print(f"  {msg}", file=sys.stderr)
    sys.exit(1)

print(f"self-check OK: all kTrig* constants bit-identical to kTrigNeon* "
      f"({neon_path})")
