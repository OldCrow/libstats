...
int main() {
    ...
    std::size_t optimal_threshold =
        stats::parallel::get_optimal_parallel_threshold("gaussian", "pdf");
    std::size_t optimal_grain = stats::parallel::get_optimal_grain_size();
    ...
    stats::parallel::safe_fill(data.begin(), data.end(), 42.0);
    ...
    stats::parallel::safe_for_each(counters.begin(), counters.end(), [](int& x) { x = 1; });
    ...
    stats::parallel::safe_sort(unsorted.begin(), unsorted.end());
    ...
    bool should_use_small = stats::parallel::should_use_parallel(10);
    bool should_use_large = stats::parallel::should_use_parallel(10000);
    bool should_use_dist = stats::parallel::should_use_distribution_parallel(1000);
    ...
    try {
        ...
        stats::parallel::safe_fill(empty_data.begin(), empty_data.end(), 1.0);
