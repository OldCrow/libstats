   ...
   protected:
    void SetUp() override {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (val > 0)
                non_exponential_data_.push_back(val);  // Keep only positive values
        }
...
TEST_F(ExponentialEnhancedTest, GoodnessOfFitTests) {
    ...
    std::cout << "  KS test (exponential data): D=" << ks_stat_exp << ", p=" << ks_p_exp
              << ", reject=" << ks_reject_exp << "\n";
    ...
    std::cout << "  KS test (non-exponential data): D=" << ks_stat_non_exp << ", p=" << ks_p_non_exp
              << ", reject=" << ks_reject_non_exp << "\n";
    ...
    std::cout << "  AD test (exponential data): A²=" << ad_stat_exp << ", p=" << ad_p_exp
              << ", reject=" << ad_reject_exp << "\n";
    std::cout << "  AD test (non-exponential data): A²=" << ad_stat_non_exp
              << ", p=" << ad_p_non_exp << ", reject=" << ad_reject_non_exp << "\n";
}
    ...
    for (const auto& op : operations) {
        ...
        if (op == "PDF") {
            ...
            if constexpr (requires {
                ...
                unitExp.getLogProbabilityWithStrategy(
                    input_span, log_output_span, stats::performance::Strategy::WORK_STEALING);
            } else {
                unitExp.getLogProbabilityWithStrategy(std::span<const double>(test_values),
                                                      std::span<double>(log_pdf_results),
                                                      stats::performance::Strategy::SCALAR);
        ...
        if (op == "PDF") {
            ...
            if constexpr (requires {
                ...
                unitExp.getProbabilityWithStrategy(
                    input_span, output_span, stats::performance::Strategy::GPU_ACCELERATED);
            } else {
                unitExp.getProbabilityWithStrategy(std::span<const double>(test_values),
                                                   std::span<double>(pdf_results),
                                                   stats::performance::Strategy::SCALAR);
...
TEST_F(ExponentialEnhancedTest, ParallelBatchFittingTests) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            ExponentialDistribution::parallelBatchFit(datasets,
