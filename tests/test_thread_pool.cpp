...
private:
    void testLevel0ConstantsIntegration() {
        ...
        ParallelUtils::parallelFor(0, smallData.size(), [&](std::size_t i) {
            smallData[i] *= 2;
        });
    ...
    void testParallelForIntegration() {
        ...
        ParallelUtils::parallelFor(0, size, [&](std::size_t i) {
            results[i] = data[i] * 2;
        });
        ...
        std::cout << "  Processed " << size << " elements in " << duration.count() << " μs" << std::endl;
    ...
    void testParallelReduceIntegration() {
        ...
        double sum = ParallelUtils::parallelReduce(0, size, 0.0,
        ...
        double product = ParallelUtils::parallelReduce(0, smallSize, 1.0,
        ...
        std::cout << "  Product test: " << product << " (expected: " << expectedProduct << ")" << std::endl;
    ...
    void testPerformanceScaling() {
        ...
        double parallelSum = ParallelUtils::parallelReduce(0, size, 0.0,
            [&](std::size_t i) { return data[i]; },
            [](double a, double b) { return a + b; });
        
        auto parallel_end = std::chrono::high_resolution_clock::now();
        auto parallel_duration = std::chrono::duration_cast<std::chrono::microseconds>(parallel_end - start);
        ...
        sequential_duration = std::chrono::duration_cast<std::chrono::microseconds>(sequential_end - start);

        start = std::chrono::high_resolution_clock::now();

        double parallelLargeSum = ParallelUtils::parallelReduce(0, largeSize, 0.0,
            [&](std::size_t i) { return largeData[i]; },
            [](double a, double b) { return a + b; });
        ...
        std::cout << "  Large dataset speedup: " << std::fixed << std::setprecision(2) << largeSpeedup << "x" << std::endl;

        // Verify both give same result
        bool largeCorrect = std::abs(sequentialLargeSum - parallelLargeSum) < 1e-6;
        std::cout << "  Large dataset results match: " << (largeCorrect ? "Yes" : "No") << std::endl;
        
        // Performance expectations
        std::cout << "  Expected speedup achieved: " << (largeSpeedup > 1.5 ? "Yes" : "No") << std::endl;
        
        assert(largeCorrect);
        
        // For multi-core systems, we should see significant speedup with large datasets
        if (ThreadPool::getOptimalThreadCount() > 1) {
            std::cout << "  Multi-core speedup validation: " << (largeSpeedup > 1.5 ? "Passed" : "May need tuning") << std::endl;
        }
    ...
    void testDocumentationExamples() {
        ...
        // Example 2: SIMD-aware parallel processing
        {
            ...
            assert(correct);
            std::cout << "  Example 2 SIMD transform: " << (correct ? "Passed" : "Failed") << std::endl;
        }
