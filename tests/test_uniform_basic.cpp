...
int main() {
    ...
    try {
        ...
        if (result.isOk()) {
            ...
            uniform_dist.getProbability(std::span<const double>(test_values), std::span<double>(pdf_results));
            ...
            start = std::chrono::high_resolution_clock::now();
            uniform_dist.getProbabilityBatch(test_values.data(), pdf_results_traditional.data(), test_values.size());
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            uniform_dist.getProbability(std::span<const double>(large_input), std::span<double>(large_output));
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            uniform_dist.getProbabilityBatch(large_input.data(), large_output_traditional.data(), large_size);
            end = std::chrono::high_resolution_clock::now();
            auto large_trad_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            
            StandardizedBasicTest::printLargeBatchValidation(large_output[0], large_output[999], "Auto-dispatch PDF at 0.5");
