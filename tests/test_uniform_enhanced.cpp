...
TEST_F(UniformEnhancedTest, SIMDAndParallelBatchImplementations) {
    ...
    for (size_t batch_size : batch_sizes) {
        ...
        stdUniform.getProbabilityBatch(test_values.data(), simd_results.data(), batch_size);
        ...
        start = std::chrono::high_resolution_clock::now();
        stdUniform.getProbabilityBatchParallel(input_span, output_span);
        end = std::chrono::high_resolution_clock::now();
        ...
        start = std::chrono::high_resolution_clock::now();
        stdUniform.getProbabilityBatchWorkStealing(input_span, ws_output_span, work_stealing_pool);
        end = std::chrono::high_resolution_clock::now();
...
TEST_F(UniformEnhancedTest, AutoDispatchAssessment) {
    ...
    for (size_t i = 0; i < batch_sizes.size(); ++i) {
        ...
        if constexpr (requires { uniform_dist.getProbability(std::span<const double>(test_values), std::span<double>(auto_pdf_results)); }) {
            ...
            uniform_dist.getProbabilityBatch(test_values.data(), auto_pdf_results.data(), batch_size);
        }
        auto end = std::chrono::high_resolution_clock::now();
        ...
        if constexpr (requires { uniform_dist.getLogProbability(std::span<const double>(test_values), std::span<double>(auto_logpdf_results)); }) {
            ...
            uniform_dist.getLogProbabilityBatch(test_values.data(), auto_logpdf_results.data(), batch_size);
        }
        end = std::chrono::high_resolution_clock::now();
        ...
        if constexpr (requires { uniform_dist.getCumulativeProbability(std::span<const double>(test_values), std::span<double>(auto_cdf_results)); }) {
            ...
            uniform_dist.getCumulativeProbabilityBatch(test_values.data(), auto_cdf_results.data(), batch_size);
        }
        end = std::chrono::high_resolution_clock::now();
...
TEST_F(UniformEnhancedTest, ParallelBatchPerformanceBenchmark) {
    ...
    for (const auto& op : operations) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        if (op == "PDF") {
            stdUniform.getProbabilityBatch(test_values.data(), pdf_results.data(), BENCHMARK_SIZE);
        } else if (op == "LogPDF") {
            stdUniform.getLogProbabilityBatch(test_values.data(), log_pdf_results.data(), BENCHMARK_SIZE);
        } else if (op == "CDF") {
            stdUniform.getCumulativeProbabilityBatch(test_values.data(), cdf_results.data(), BENCHMARK_SIZE);
        }
        auto end = std::chrono::high_resolution_clock::now();
        ...
        if (op == "PDF") {
            ...
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getProbabilityBatchParallel(input_span, output_span); }) {
                stdUniform.getProbabilityBatchParallel(input_span, output_span);
            } else {
                stdUniform.getProbabilityBatch(test_values.data(), pdf_results.data(), BENCHMARK_SIZE);
            }
            ...
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getLogProbabilityBatchParallel(input_span, log_output_span); }) {
                stdUniform.getLogProbabilityBatchParallel(input_span, log_output_span);
            } else {
                stdUniform.getLogProbabilityBatch(test_values.data(), log_pdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getCumulativeProbabilityBatchParallel(input_span, cdf_output_span); }) {
                stdUniform.getCumulativeProbabilityBatchParallel(input_span, cdf_output_span);
            } else {
                stdUniform.getCumulativeProbabilityBatch(test_values.data(), cdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
        ...
        if (op == "PDF") {
            std::span<double> output_span(pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getProbabilityBatchWorkStealing(input_span, output_span, work_stealing_pool); }) {
                stdUniform.getProbabilityBatchWorkStealing(input_span, output_span, work_stealing_pool);
            } else {
                stdUniform.getProbabilityBatch(test_values.data(), pdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> log_output_span(log_pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getLogProbabilityBatchWorkStealing(input_span, log_output_span, work_stealing_pool); }) {
                stdUniform.getLogProbabilityBatchWorkStealing(input_span, log_output_span, work_stealing_pool);
            } else {
                stdUniform.getLogProbabilityBatch(test_values.data(), log_pdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> cdf_output_span(cdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getCumulativeProbabilityBatchWorkStealing(input_span, cdf_output_span, work_stealing_pool); }) {
                stdUniform.getCumulativeProbabilityBatchWorkStealing(input_span, cdf_output_span, work_stealing_pool);
            } else {
                stdUniform.getCumulativeProbabilityBatch(test_values.data(), cdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
        ...
        if (op == "PDF") {
            std::span<double> output_span(pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { typename cache::AdaptiveCache<std::string, double>; stdUniform.getProbabilityBatchCacheAware(input_span, output_span, std::declval<cache::AdaptiveCache<std::string, double>&>()); }) {
                stdUniform.getProbabilityBatchCacheAware(input_span, output_span, cache_manager);
            } else {
                stdUniform.getProbabilityBatch(test_values.data(), pdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> log_output_span(log_pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { typename cache::AdaptiveCache<std::string, double>; stdUniform.getLogProbabilityBatchCacheAware(input_span, log_output_span, std::declval<cache::AdaptiveCache<std::string, double>&>()); }) {
                stdUniform.getLogProbabilityBatchCacheAware(input_span, log_output_span, cache_manager);
            } else {
                stdUniform.getLogProbabilityBatch(test_values.data(), log_pdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> cdf_output_span(cdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { typename cache::AdaptiveCache<std::string, double>; stdUniform.getCumulativeProbabilityBatchCacheAware(input_span, cdf_output_span, std::declval<cache::AdaptiveCache<std::string, double>&>()); }) {
                stdUniform.getCumulativeProbabilityBatchCacheAware(input_span, cdf_output_span, cache_manager);
            } else {
                stdUniform.getCumulativeProbabilityBatch(test_values.data(), cdf_results.data(), BENCHMARK_SIZE);
            }
            end = std::chrono::high_resolution_clock::now();
