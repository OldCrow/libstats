...
TEST_F(UniformEnhancedTest, GoodnessOfFitTests) {
    ...
    std::cout << "  AD test (uniform data): A²=" << ad_stat_uniform << ", p=" << ad_p_uniform << ", reject=" << ad_reject_uniform << "\n";
    std::cout << "  AD test (non-uniform data): A²=" << ad_stat_non_uniform << ", p=" << ad_p_non_uniform << ", reject=" << ad_reject_non_uniform << "\n";
}
...
TEST_F(UniformEnhancedTest, BootstrapMethods) {
    ...
    auto [a_ci_pair, b_ci_pair] = UniformDistribution::bootstrapParameterConfidenceIntervals(
    ...
    auto [loocv_mae, loocv_rmse, loocv_log_likelihood] = UniformDistribution::leaveOneOutCrossValidation(small_uniform_data);
...
TEST_F(UniformEnhancedTest, SIMDAndParallelBatchImplementations) {
    ...
    for (size_t batch_size : batch_sizes) {
        ...
        stdUniform.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(simd_results), libstats::performance::Strategy::SCALAR);
        ...
        start = std::chrono::high_resolution_clock::now();
        stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::PARALLEL_SIMD);
        end = std::chrono::high_resolution_clock::now();
        ...
        start = std::chrono::high_resolution_clock::now();
        stdUniform.getProbabilityWithStrategy(input_span, ws_output_span, libstats::performance::Strategy::WORK_STEALING);
        end = std::chrono::high_resolution_clock::now();
        ...
        if (std::thread::hardware_concurrency() > 1) {
            if (batch_size >= 10000) {
                // For uniform distributions, computations are extremely simple (range checks and constants),
                // so SIMD can achieve massive speedups (50-70x) but parallel has thread overhead.
                // Expect parallel to be at least 70% as efficient as SIMD for large batches.
                EXPECT_GT(parallel_speedup, simd_speedup * 0.7) << "Parallel should be reasonably competitive with SIMD for large batches";
            } else {
...
TEST_F(UniformEnhancedTest, AdvancedStatisticalMethods) {
    ...
    auto [a_lower, a_upper] = UniformDistribution::confidenceIntervalLowerBound(uniform_data_, 0.95);
    auto [b_lower, b_upper] = UniformDistribution::confidenceIntervalUpperBound(uniform_data_, 0.95);
    ...
    auto [lr_stat, p_value, reject_null] = UniformDistribution::likelihoodRatioTest(uniform_data_, test_a_, test_b_, 0.05);
...
TEST_F(UniformEnhancedTest, AutoDispatchAssessment) {
    ...
    for (size_t i = 0; i < batch_sizes.size(); ++i) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        if constexpr (requires { uniform_dist.getProbability(std::span<const double>(test_values), std::span<double>(auto_pdf_results)); }) {
            uniform_dist.getProbability(std::span<const double>(test_values), std::span<double>(auto_pdf_results));
        } else {
            uniform_dist.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(auto_pdf_results), libstats::performance::Strategy::SCALAR);
        }
        auto end = std::chrono::high_resolution_clock::now();
        auto auto_pdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        
        start = std::chrono::high_resolution_clock::now();
        if constexpr (requires { uniform_dist.getLogProbability(std::span<const double>(test_values), std::span<double>(auto_logpdf_results)); }) {
            uniform_dist.getLogProbability(std::span<const double>(test_values), std::span<double>(auto_logpdf_results));
        } else {
            uniform_dist.getLogProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(auto_logpdf_results), libstats::performance::Strategy::SCALAR);
        }
        end = std::chrono::high_resolution_clock::now();
        auto auto_logpdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        
        start = std::chrono::high_resolution_clock::now();
        if constexpr (requires { uniform_dist.getCumulativeProbability(std::span<const double>(test_values), std::span<double>(auto_cdf_results)); }) {
            uniform_dist.getCumulativeProbability(std::span<const double>(test_values), std::span<double>(auto_cdf_results));
        } else {
            uniform_dist.getCumulativeProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(auto_cdf_results), libstats::performance::Strategy::SCALAR);
        }
        end = std::chrono::high_resolution_clock::now();
        ...
        uniform_dist.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(trad_pdf_results), libstats::performance::Strategy::SCALAR);
        uniform_dist.getLogProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(trad_logpdf_results), libstats::performance::Strategy::SCALAR);
        uniform_dist.getCumulativeProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(trad_cdf_results), libstats::performance::Strategy::SCALAR);
...
TEST_F(UniformEnhancedTest, ParallelBatchPerformanceBenchmark) {
    ...
    for (const auto& op : operations) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        if (op == "PDF") {
            stdUniform.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(pdf_results), libstats::performance::Strategy::SCALAR);
        } else if (op == "LogPDF") {
            stdUniform.getLogProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(log_pdf_results), libstats::performance::Strategy::SCALAR);
        } else if (op == "CDF") {
            stdUniform.getCumulativeProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(cdf_results), libstats::performance::Strategy::SCALAR);
        }
        auto end = std::chrono::high_resolution_clock::now();
        ...
        if (op == "PDF") {
            ...
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::PARALLEL_SIMD); }) {
                stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::PARALLEL_SIMD);
            } else {
                stdUniform.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(pdf_results), libstats::performance::Strategy::SCALAR);
            }
            ...
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getLogProbabilityWithStrategy(input_span, log_output_span, libstats::performance::Strategy::PARALLEL_SIMD); }) {
                stdUniform.getLogProbabilityWithStrategy(input_span, log_output_span, libstats::performance::Strategy::PARALLEL_SIMD);
            } else {
                stdUniform.getLogProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(log_pdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getCumulativeProbabilityWithStrategy(input_span, cdf_output_span, libstats::performance::Strategy::PARALLEL_SIMD); }) {
                stdUniform.getCumulativeProbabilityWithStrategy(input_span, cdf_output_span, libstats::performance::Strategy::PARALLEL_SIMD);
            } else {
                stdUniform.getCumulativeProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(cdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
        ...
        if (op == "PDF") {
            std::span<double> output_span(pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::WORK_STEALING); }) {
                stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::WORK_STEALING);
            } else {
                stdUniform.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(pdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> log_output_span(log_pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getLogProbabilityWithStrategy(input_span, log_output_span, libstats::performance::Strategy::WORK_STEALING); }) {
                stdUniform.getLogProbabilityWithStrategy(input_span, log_output_span, libstats::performance::Strategy::WORK_STEALING);
            } else {
                stdUniform.getLogProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(log_pdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> cdf_output_span(cdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { stdUniform.getCumulativeProbabilityWithStrategy(input_span, cdf_output_span, libstats::performance::Strategy::WORK_STEALING); }) {
                stdUniform.getCumulativeProbabilityWithStrategy(input_span, cdf_output_span, libstats::performance::Strategy::WORK_STEALING);
            } else {
                stdUniform.getCumulativeProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(cdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
        ...
        if (op == "PDF") {
            std::span<double> output_span(pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { typename cache::AdaptiveCache<std::string, double>; stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::GPU_ACCELERATED); }) {
                stdUniform.getProbabilityWithStrategy(input_span, output_span, libstats::performance::Strategy::GPU_ACCELERATED);
            } else {
                stdUniform.getProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(pdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> log_output_span(log_pdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { typename cache::AdaptiveCache<std::string, double>; stdUniform.getLogProbabilityWithStrategy(input_span, log_output_span, libstats::performance::Strategy::GPU_ACCELERATED); }) {
                stdUniform.getLogProbabilityWithStrategy(input_span, log_output_span, libstats::performance::Strategy::GPU_ACCELERATED);
            } else {
                stdUniform.getLogProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(log_pdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
            ...
            std::span<double> cdf_output_span(cdf_results);
            start = std::chrono::high_resolution_clock::now();
            if constexpr (requires { typename cache::AdaptiveCache<std::string, double>; stdUniform.getCumulativeProbabilityWithStrategy(input_span, cdf_output_span, libstats::performance::Strategy::GPU_ACCELERATED); }) {
                stdUniform.getCumulativeProbabilityWithStrategy(input_span, cdf_output_span, libstats::performance::Strategy::GPU_ACCELERATED);
            } else {
                stdUniform.getCumulativeProbabilityWithStrategy(std::span<const double>(test_values), std::span<double>(cdf_results), libstats::performance::Strategy::SCALAR);
            }
            end = std::chrono::high_resolution_clock::now();
        ...
        // Verify correctness
        if (op == "PDF") {
            StatisticalTestUtils::verifyBatchCorrectness(stdUniform, test_values, pdf_results, "PDF");
        } else if (op == "LogPDF") {
            StatisticalTestUtils::verifyBatchCorrectness(stdUniform, test_values, log_pdf_results, "LogPDF");
        } else if (op == "CDF") {
            StatisticalTestUtils::verifyBatchCorrectness(stdUniform, test_values, cdf_results, "CDF");
        }
    }
