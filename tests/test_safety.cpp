...
void test_safe_log_basic() {
    ...
    test_assert(safe_log(std::numeric_limits<double>::quiet_NaN()) == 
              prob::MIN_LOG_PROBABILITY, "safe_log(NaN) edge case");
    test_assert(safe_log(std::numeric_limits<double>::infinity()) == 
              std::numeric_limits<double>::max(), "safe_log(inf) edge case");
}
...
void test_safe_exp_basic() {
    ...
    // Edge cases
    test_assert(safe_exp(std::numeric_limits<double>::quiet_NaN()) == 0.0, "safe_exp(NaN) edge case");
...
void test_safe_sqrt_basic() {
    ...
    test_assert(safe_sqrt(std::numeric_limits<double>::quiet_NaN()) == 0.0, "safe_sqrt(NaN) edge case");
    test_assert(safe_sqrt(std::numeric_limits<double>::infinity()) == 
              std::numeric_limits<double>::max(), "safe_sqrt(inf) edge case");
}
...
void test_clamp_probability_basic() {
    ...
    test_assert(clamp_probability(std::numeric_limits<double>::quiet_NaN()) == 
              prob::MIN_PROBABILITY, "clamp_probability(NaN) edge case");
}
...
void test_clamp_log_probability_basic() {
    ...
    test_assert(clamp_log_probability(std::numeric_limits<double>::quiet_NaN()) == 
              prob::MIN_LOG_PROBABILITY, "clamp_log_probability(NaN) edge case");
}
...
void test_vector_safe_sqrt_small_array() {
    ...
    std::vector<double> input = {0.0, 1.0, 4.0, 9.0, -1.0, 
                                 std::numeric_limits<double>::quiet_NaN()};
    std::vector<double> output(input.size());
...
void test_vector_clamp_probability_small_array() {
    ...
    std::vector<double> input = {0.5, 0.0, 1.0, -0.5, 1.5, 
                                 std::numeric_limits<double>::quiet_NaN()};
    std::vector<double> output(input.size());
...
void test_vector_clamp_log_probability_small_array() {
    ...
    std::vector<double> input = {-0.5, 0.0, 0.5, -1000.0, 
                                 std::numeric_limits<double>::quiet_NaN()};
    std::vector<double> output(input.size());
    ...
    test_assert(output[1] == prob::MAX_LOG_PROBABILITY, "vector_clamp_log_probability[1] edge case");
    test_assert(output[2] == prob::MAX_LOG_PROBABILITY, "vector_clamp_log_probability[2] edge case");
    test_assert(output[3] == -1000.0, "vector_clamp_log_probability[3] should be unchanged");
    test_assert(output[4] == prob::MIN_LOG_PROBABILITY, "vector_clamp_log_probability[4] edge case");
}
