    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            for (int i = 0; i < accesses_per_thread && thread_success; ++i) {
                ...
                // Verify consistency
                if (caps.logical_cores() == 0) thread_success = false;
                if (caps.physical_cores() == 0) thread_success = false;
                if (caps.physical_cores() > caps.logical_cores()) thread_success = false;
                if (caps.l1_cache_size() == 0) thread_success = false;
                
                // Verify SIMD consistency
                if (caps.has_avx2() && !caps.has_avx()) thread_success = false;
                if (caps.has_avx() && !caps.has_sse2()) thread_success = false;
