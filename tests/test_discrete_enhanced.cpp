...
TEST_F(DiscreteEnhancedTest, SmartAutoDispatchStrategyTesting) {
    ...
    std::cout << "\n=== Smart Auto-Dispatch Strategy Testing (Discrete) ===\n";
    ...
    for (size_t i = 0; i < batch_sizes.size(); ++i) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        discrete_dist.getProbability(std::span<const double>(test_values), std::span<double>(auto_pdf_results));
        auto end = std::chrono::high_resolution_clock::now();
        auto auto_pdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        
        start = std::chrono::high_resolution_clock::now();
        discrete_dist.getLogProbability(std::span<const double>(test_values), std::span<double>(auto_logpdf_results));
        end = std::chrono::high_resolution_clock::now();
        ...
        discrete_dist.getProbabilityBatch(test_values.data(), trad_pdf_results.data(), batch_size);
        discrete_dist.getLogProbabilityBatch(test_values.data(), trad_logpdf_results.data(), batch_size);
    ...
    std::cout << "\n=== Auto-Dispatch Strategy Testing Completed (Discrete) ===\n";
}
