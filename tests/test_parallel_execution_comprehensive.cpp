...
int main() {
    ...
    [[maybe_unused]] auto count_result =
        libstats::parallel::safe_count(count_data.begin(), count_data.end(), 2);
    assert(count_result == 4);
    std::cout << "  - safe_count (GCD): PASSED" << std::endl;

    // Test safe_count_if with GCD
    [[maybe_unused]] auto count_if_result = libstats::parallel::safe_count_if(
    ...
    [[maybe_unused]] auto sum_result =
        libstats::parallel::safe_reduce(reduce_data.begin(), reduce_data.end(), 0.0);
    ...
    [[maybe_unused]] auto large_sum =
        libstats::parallel::safe_reduce(large_data.begin(), large_data.end(), 0.0);
    ...
    libstats::parallel::safe_transform(test_data.begin(), test_data.end(),
    ...
    [[maybe_unused]] auto perf_sum_result =
        libstats::parallel::safe_reduce(perf_data.begin(), perf_data.end(), 0.0);
    ...
    try {
        ...
        [[maybe_unused]] auto empty_count =
            libstats::parallel::safe_count(empty.begin(), empty.end(), 5);
        ...
        [[maybe_unused]] auto null_count =
            libstats::parallel::safe_count(ptr_vec.begin(), ptr_vec.end(), nullptr);
    ...
    auto compute_grain =
        libstats::parallel::get_adaptive_grain_size(1, 10000);                 // Computation-bound
    ...
    std::cout << "  - Standard execution policies available: " << (has_std_exec ? "YES" : "NO")
              << std::endl;
    std::cout << "  - Execution support: " << libstats::parallel::execution_support_string()
              << std::endl;
    ...
    std::cout << "    * 10 elements: "
              << (libstats::parallel::should_use_parallel(10) ? "parallel" : "serial") << std::endl;
    std::cout << "    * 100 elements: "
              << (libstats::parallel::should_use_parallel(100) ? "parallel" : "serial")
              << std::endl;
    std::cout << "    * 1000 elements: "
              << (libstats::parallel::should_use_parallel(1000) ? "parallel" : "serial")
              << std::endl;
    std::cout << "    * 10000 elements: "
              << (libstats::parallel::should_use_parallel(10000) ? "parallel" : "serial")
              << std::endl;
    ...
    auto gpu_accelerated_grain =
        libstats::parallel::get_adaptive_grain_size(0, 1000000);  // 1M elements
