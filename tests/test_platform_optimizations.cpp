...
int main() {
    ...
    bool tiny_decision = VectorOps::should_use_vectorized_path(
        test_data_tiny.size(), test_data_tiny.data());
    
    // For medium and large data, should generally use vectorization
    bool medium_decision = VectorOps::should_use_vectorized_path(
        test_data_medium.size(), test_data_medium.data());
    bool large_decision = VectorOps::should_use_vectorized_path(
        test_data_large.size(), test_data_large.data());
    ...
    std::cout << "  - Small data (8 elements): " << (VectorOps::should_use_vectorized_path(test_data_small.size(), test_data_small.data()) ? "SIMD" : "Scalar") << std::endl;
    std::cout << "  - Medium data (64 elements): " << (medium_decision ? "SIMD" : "Scalar") << std::endl;
    std::cout << "  - Large data (2000 elements): " << (large_decision ? "SIMD" : "Scalar") << std::endl;
