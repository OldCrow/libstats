...
void test_constant_relationships() {
    ...
    assert(memory::access::bandwidth::DDR5_BURST_SIZE >= memory::access::bandwidth::DDR4_BURST_SIZE);
...
void print_summary() {
    ...
    std::cout << "  Apple Silicon: " << memory::prefetch::platform::apple_silicon::SEQUENTIAL_PREFETCH_DISTANCE << std::endl;
    std::cout << "  Intel: " << memory::prefetch::platform::intel::SEQUENTIAL_PREFETCH_DISTANCE << std::endl;
    std::cout << "  AMD: " << memory::prefetch::platform::amd::SEQUENTIAL_PREFETCH_DISTANCE << std::endl;
    std::cout << "  ARM: " << memory::prefetch::platform::arm::SEQUENTIAL_PREFETCH_DISTANCE << std::endl;
    
    std::cout << "\nMemory Access:" << std::endl;
    std::cout << "  Cache line size: " << memory::access::CACHE_LINE_SIZE_BYTES << " bytes" << std::endl;
    std::cout << "  Doubles per cache line: " << memory::access::DOUBLES_PER_CACHE_LINE << std::endl;
    std::cout << "  NUMA aware threshold: " << (memory::access::numa::NUMA_AWARE_THRESHOLD / 1024 / 1024) << " MB" << std::endl;
}
...
int main() {
    ...
    try {
        ...
        std::cout << "📊 SIMD alignment, matrix blocking, and memory prefetching constants verified! 📊" << std::endl;
