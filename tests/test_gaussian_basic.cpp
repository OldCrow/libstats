...
int main() {
    ...
    try {
        ...
        if (result.isOk()) {
            ...
            gauss_dist.getProbability(std::span<const double>(test_values), std::span<double>(pdf_results));
            auto end = std::chrono::high_resolution_clock::now();
            auto auto_pdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getLogProbability(std::span<const double>(test_values), std::span<double>(log_pdf_results));
            end = std::chrono::high_resolution_clock::now();
            auto auto_logpdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getCumulativeProbability(std::span<const double>(test_values), std::span<double>(cdf_results));
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getProbabilityBatch(test_values.data(), pdf_results_traditional.data(), test_values.size());
            end = std::chrono::high_resolution_clock::now();
            auto trad_pdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getLogProbabilityBatch(test_values.data(), log_pdf_results_traditional.data(), test_values.size());
            end = std::chrono::high_resolution_clock::now();
            auto trad_logpdf_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getCumulativeProbabilityBatch(test_values.data(), cdf_results_traditional.data(), test_values.size());
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getProbability(std::span<const double>(large_input), std::span<double>(large_output));
            end = std::chrono::high_resolution_clock::now();
            ...
            start = std::chrono::high_resolution_clock::now();
            gauss_dist.getProbabilityBatch(large_input.data(), large_output_traditional.data(), large_size);
            end = std::chrono::high_resolution_clock::now();
            auto large_trad_time = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            
            StandardizedBasicTest::printLargeBatchValidation(large_output[0], large_output[999], "Auto-dispatch PDF at 0");
