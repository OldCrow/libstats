...
TEST_F(PerformanceInitializationTest, BatchOperationsWorkAfterInitialization) {
    ...
    std::cout << "Batch operation duration: " << duration << " μs for " << test_size << " values"
              << std::endl;
...
TEST(PerformanceInitializationBenchmark, InitializationImprovesColdStartPerformance) {
    ...
    auto duration_with_init =
        std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();

    std::cout << "Batch operation with initialization: " << duration_with_init << " μs"
              << std::endl;
