...
TEST_F(PerformanceDispatcherTest, BasicStrategySelection) {
    ...
    auto strategy_small = dispatcher.selectOptimalStrategy(
        5, DistributionType::GAUSSIAN, ComputationComplexity::SIMPLE, system);
    EXPECT_EQ(strategy_small, Strategy::SCALAR);
    
    // Very large batches should prefer parallel strategies
    auto strategy_large = dispatcher.selectOptimalStrategy(
...
TEST_F(PerformanceDispatcherTest, DistributionSpecificThresholds) {
    ...
    auto uniform_medium = dispatcher.selectOptimalStrategy(
        1000, DistributionType::UNIFORM, ComputationComplexity::SIMPLE, system);
...
TEST_F(PerformanceDispatcherTest, ComplexityInfluencesStrategy) {
    ...
    auto simple_strategy = dispatcher.selectOptimalStrategy(
        batch_size, dist, ComputationComplexity::SIMPLE, system);
    
    [[maybe_unused]] auto complex_strategy = dispatcher.selectOptimalStrategy(
        batch_size, dist, ComputationComplexity::COMPLEX, system);
...
TEST_F(PerformanceDispatcherTest, PerformanceRecording) {
    ...
    PerformanceDispatcher::recordPerformance(
        Strategy::SIMD_BATCH, DistributionType::EXPONENTIAL, 500, 1500);
    PerformanceDispatcher::recordPerformance(
        Strategy::SCALAR, DistributionType::EXPONENTIAL, 500, 2000);
    
    // Verify data was recorded in the global history
    auto& history = PerformanceDispatcher::getPerformanceHistory();
    
    auto simd_stats = history.getPerformanceStats(Strategy::SIMD_BATCH, DistributionType::EXPONENTIAL);
    auto scalar_stats = history.getPerformanceStats(Strategy::SCALAR, DistributionType::EXPONENTIAL);
...
TEST_F(PerformanceDispatcherTest, EdgeCases) {
    ...
    auto zero_strategy = dispatcher.selectOptimalStrategy(
        0, DistributionType::GAUSSIAN, ComputationComplexity::SIMPLE, system);
    EXPECT_EQ(zero_strategy, Strategy::SCALAR);
    
    // Single element
    auto single_strategy = dispatcher.selectOptimalStrategy(
        1, DistributionType::GAMMA, ComputationComplexity::COMPLEX, system);
    EXPECT_EQ(single_strategy, Strategy::SCALAR);
    
    // Extremely large batch size
    auto huge_strategy = dispatcher.selectOptimalStrategy(
    ...
    for (std::size_t t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            for (std::size_t i = 0; i < selections_per_thread; ++i) {
                ...
                auto strategy = dispatcher.selectOptimalStrategy(batch_size, dist_type, complexity, system);
