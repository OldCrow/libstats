...
void demonstrate_simd_integration() {
    ...
    std::cout << "   - SSE2: " << (stats::arch::supports_sse2() ? "✓" : "✗") << std::endl;
    std::cout << "   - SSE4.1: " << (stats::arch::supports_sse4_1() ? "✓" : "✗") << std::endl;
    std::cout << "   - AVX: " << (stats::arch::supports_avx() ? "✓" : "✗") << std::endl;
    std::cout << "   - AVX2: " << (stats::arch::supports_avx2() ? "✓" : "✗") << std::endl;
    std::cout << "   - FMA: " << (stats::arch::supports_fma() ? "✓" : "✗") << std::endl;
    std::cout << "   - AVX512: " << (stats::arch::supports_avx512() ? "✓" : "✗") << std::endl;

    std::cout << "   ARM SIMD Support:" << std::endl;
    std::cout << "   - NEON: " << (stats::arch::supports_neon() ? "✓" : "✗") << std::endl;

    // 3. Show optimal settings
    std::cout << "\n3. OPTIMAL SIMD CONFIGURATION:" << std::endl;
    std::cout << "   Best SIMD level: " << stats::arch::best_simd_level() << std::endl;
    std::cout << "   Optimal double width: " << stats::arch::optimal_double_width() << std::endl;
    std::cout << "   Optimal float width: " << stats::arch::optimal_float_width() << std::endl;
    std::cout << "   Optimal alignment: " << stats::arch::optimal_alignment() << " bytes"
              << std::endl;
    ...
    if (!has_simd) {
        ...
        if (features.vendor == "Apple" || features.brand.find("Apple") != std::string::npos) {
            std::cout
                << "   ℹ️ x86 SIMD not available on Apple Silicon - this is expected and normal"
                << std::endl;
#ifdef LIBSTATS_HAS_NEON
            std::cout << "   ✓ ARM NEON SIMD is the optimal choice for this platform" << std::endl;
#else
            std::cout << "   ⚠ ARM NEON support not compiled in - using scalar fallback"
                      << std::endl;
#endif
        } else {
...
void safe_vectorized_add(const std::vector<double>& a, const std::vector<double>& b,
...
void benchmark_gaussian_simd() {
    ...
    std::cout << "PDF batch operations (100 iterations): " << duration_pdf.count()
    ...
    std::cout << "Log PDF batch operations (100 iterations): " << duration_log.count()
              << " microseconds" << std::endl;
    ...
    std::cout << "Log PDF batch verification: " << (log_pdf_correct ? "✓ PASSED" : "✗ FAILED")
              << std::endl;
    ...
    for (size_t i = 0; i < 5; ++i) {
        std::cout << "x=" << std::setw(8) << std::fixed << std::setprecision(3) << values[i]
                  ...
                  << " logPDF=" << std::setw(12) << std::setprecision(6) << log_pdf_results[i]
                  << std::endl;
    }
}
...
void benchmark_simd_performance() {
    ...
    std::cout << "Time for 100 iterations of " << size
              << " element vector addition: " << duration.count() << " microseconds" << std::endl;
