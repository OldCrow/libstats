...
void demonstrate_parameter_validation() {
    ...
    if (invalid_uniform_result.isOk()) {
        ...
        std::cout << "✅ Correctly rejected invalid parameters: " << invalid_uniform_result.message()
                  << std::endl;
    }
    ...
    if (negative_gaussian_result.isOk()) {
        ...
        std::cout << "✅ Correctly rejected negative σ: " << negative_gaussian_result.message()
                  << std::endl;
    }
    ...
    if (negative_poisson_result.isOk()) {
        ...
        std::cout << "✅ Correctly rejected negative λ: " << negative_poisson_result.message()
                  << std::endl;
    }

    std::cout << "\n💡 Best Practice: Always validate parameters before creating distributions!"
              << std::endl;
}
...
void demonstrate_safe_creation_patterns() {
    ...
    auto safe_create_uniform = [](double a, double b) -> std::unique_ptr<stats::Uniform> {
        ...
        if (result.isOk()) {
            ...
            return std::make_unique<stats::Uniform>(std::move(result).unwrap());
        } else {
    ...
    if (auto dist = safe_create_uniform(0.0, 10.0)) {
        ...
        std::cout << "   Mean: " << std::fixed << std::setprecision(3) << dist->getMean()
                  << std::endl;
    }
    ...
    try {
        ...
        auto exp_dist = stats::ExponentialDistribution::create(rate).unwrap();
    ...
    try {
        ...
        auto exp_dist = stats::ExponentialDistribution::create(invalid_rate).unwrap();
...
void demonstrate_numerical_edge_cases() {
    ...
    try {
        auto tiny_std = stats::GaussianDistribution::create(0.0, 1e-10).unwrap();
    ...
    try {
        auto large_std = stats::GaussianDistribution::create(0.0, 1e6).unwrap();
    ...
    try {
        auto high_rate = stats::PoissonDistribution::create(1000.0).unwrap();
    ...
    auto unit_uniform = stats::UniformDistribution::create(0.0, 1.0).unwrap();
    ...
    std::cout << "   PDF at x = 0.0 (lower bound): " << unit_uniform.getProbability(0.0)
              << std::endl;
    std::cout << "   PDF at x = 1.0 (upper bound): " << unit_uniform.getProbability(1.0)
              << std::endl;
    std::cout << "   PDF at x = -0.001 (below range): " << unit_uniform.getProbability(-0.001)
              << std::endl;
    std::cout << "   PDF at x = 1.001 (above range): " << unit_uniform.getProbability(1.001)
              << std::endl;
...
void demonstrate_sampling_safety() {
    ...
    auto gaussian = stats::GaussianDistribution::create(0.0, 1.0).unwrap();
    ...
    auto safe_sample = [](auto& dist, std::mt19937& rng, int n_samples) -> std::vector<double> {
        ...
        try {
            ...
            std::cerr << "Sampling error after " << samples.size() << " samples: " << e.what()
                      << std::endl;
    ...
    try {
        ...
        std::cout << "   Sample mean: " << std::fixed << std::setprecision(4)
                  << sum / static_cast<double>(batch_samples.size() -
                                               static_cast<std::size_t>(nan_count) -
                                               static_cast<std::size_t>(inf_count))
                  << std::endl;
    ...
    auto time_seed = static_cast<std::uint_fast32_t>(
...
void demonstrate_input_validation() {
    ...
    for (double x : test_inputs) {
        ...
        if (validate_input(x, "PDF evaluation")) {
            try {
                ...
                std::cout << "   PDF result: " << std::scientific << std::setprecision(3)
                          << pdf_result << std::endl;

                double cdf_result = gaussian.getCumulativeProbability(x);
                std::cout << "   CDF result: " << std::fixed << std::setprecision(6) << cdf_result
                          << std::endl;
            } catch (const std::exception& e) {
    ...
    std::vector<double> probability_inputs = {
        0.0, 0.25, 0.5, 0.75, 1.0, -0.1, 1.1, std::numeric_limits<double>::quiet_NaN()};
    ...
    for (double p : probability_inputs) {
        ...
        try {
            if (p >= 0.0 && p <= 1.0 && !std::isnan(p)) {
                ...
                std::cout << "✅ Quantile: " << std::fixed << std::setprecision(4) << quantile
                          << std::endl;
            } else {
...
void demonstrate_recovery_strategies() {
    ...
    auto create_distribution_with_fallback =
        ...
        if (result.isOk()) {
            return std::make_unique<stats::Gaussian>(std::move(result).unwrap());
        } else {
            ...
            if (fallback.isOk()) {
                return std::make_unique<stats::Gaussian>(std::move(fallback).unwrap());
            } else {
...
int main() {
    ...
    try {
        ...
        std::cout << "\n🛡️  Safe programming leads to reliable statistical applications!"
                  << std::endl;
