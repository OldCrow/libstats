...
void compare_statistical_properties() {
    ...
    std::cout << "Uniform(0,10)   | " 
              << std::setw(7) << uniform.getMean() << " | "
              << std::setw(8) << uniform.getVariance() << " | "
              << std::setw(8) << libstats::getStandardDeviation(uniform) << " | "
              << std::setw(8) << uniform.getSkewness() << " | "
              << std::setw(8) << uniform.getKurtosis() << " | "
              << "[0, 10]" << std::endl;
    
    std::cout << "Poisson(λ=5)    | " 
              << std::setw(7) << poisson.getMean() << " | "
              << std::setw(8) << poisson.getVariance() << " | "
              << std::setw(8) << libstats::getStandardDeviation(poisson) << " | "
              << std::setw(8) << poisson.getSkewness() << " | "
              << std::setw(8) << poisson.getKurtosis() << " | "
              << "[0, ∞)" << std::endl;
    
    std::cout << "Exponential(0.2)| " 
              << std::setw(7) << exponential.getMean() << " | "
              << std::setw(8) << exponential.getVariance() << " | "
              << std::setw(8) << libstats::getStandardDeviation(exponential) << " | "
              << std::setw(8) << exponential.getSkewness() << " | "
              << std::setw(8) << exponential.getKurtosis() << " | "
              << "[0, ∞)" << std::endl;
    
    std::cout << "Gaussian(5,2)   | " 
              << std::setw(7) << gaussian.getMean() << " | "
              << std::setw(8) << gaussian.getVariance() << " | "
              << std::setw(8) << libstats::getStandardDeviation(gaussian) << " | "
              << std::setw(8) << gaussian.getSkewness() << " | "
              << std::setw(8) << gaussian.getKurtosis() << " | "
              << "(-∞, ∞)" << std::endl;
    
    std::cout << "\n📊 Key observations:" << std::endl;
    std::cout << "   • Uniform: Zero skewness (symmetric), platykurtic (negative excess kurtosis)" << std::endl;
    ...
    std::cout << "   • Gaussian: Zero skewness (symmetric), mesokurtic (zero excess kurtosis)" << std::endl;
}
...
void compare_probability_functions() {
    ...
    std::cout << "  Uniform(0,10):      f(" << x << ") = " << uniform.getProbability(x) << std::endl;
    std::cout << "  Exponential(0.2):   f(" << x << ") = " << exponential.getProbability(x) << std::endl;
    std::cout << "  Gaussian(5,2):      f(" << x << ") = " << gaussian.getProbability(x) << std::endl;
    ...
    std::cout << "  Uniform(0,10):      " << std::setprecision(4) << uniform.getCumulativeProbability(cdf_point) << std::endl;
    std::cout << "  Exponential(0.2):   " << exponential.getCumulativeProbability(cdf_point) << std::endl;
    std::cout << "  Gaussian(5,2):      " << gaussian.getCumulativeProbability(cdf_point) << std::endl;
...
void compare_sampling_behavior() {
    ...
    std::cout << "----------------|-------------|------------|----------------" << std::endl;
    std::cout << "Uniform(0,10)   | " << std::setw(11) << u_mean << " | " << std::setw(10) << u_std << " | [" << std::setprecision(1) << u_min << ", " << u_max << "]" << std::endl;
    std::cout << "Exponential(0.2)| " << std::setw(11) << std::setprecision(3) << e_mean << " | " << std::setw(10) << e_std << " | [" << std::setprecision(1) << e_min << ", " << e_max << "]" << std::endl;
    std::cout << "Gaussian(5,2)   | " << std::setw(11) << std::setprecision(3) << g_mean << " | " << std::setw(10) << g_std << " | [" << std::setprecision(1) << g_min << ", " << g_max << "]" << std::endl;
    std::cout << "Poisson(λ=5)    | " << std::setw(11) << std::setprecision(3) << p_mean << " | " << std::setw(10) << p_std << " | [" << static_cast<int>(p_min) << ", " << static_cast<int>(p_max) << "]" << std::endl;
    
    std::cout << "\n📈 Sample behavior insights:" << std::endl;
    std::cout << "   • Uniform: Bounded samples, consistent spread" << std::endl;
    std::cout << "   • Exponential: Unbounded, occasional large values (heavy right tail)" << std::endl;
...
void demonstrate_real_world_scenarios() {
    ...
    std::cout << "   P(response > 20 min): " << (1.0 - service_normal.getCumulativeProbability(20.0)) << std::endl;
    ...
    std::cout << "   P(≥ 12 events): " << (1.0 - daily_events.getCumulativeProbability(11)) << std::endl;
    ...
    std::cout << "   P(within spec): " << std::setprecision(4) 
              << (process_normal.getCumulativeProbability(50.2) - process_normal.getCumulativeProbability(49.8)) << std::endl;
    std::cout << "   Standard deviation: " << libstats::getStandardDeviation(process_normal) << std::endl;
}
...
void compare_performance() {
    ...
    std::cout << "\n⚡ Fastest sampling: " << fastest.first 
              << " (" << fastest.second << " μs total)" << std::endl;
...
int main() {
    ...
    try {
        ...
        std::cout << "\n🎯 This comparison helps in selecting the right distribution for your data!" << std::endl;
