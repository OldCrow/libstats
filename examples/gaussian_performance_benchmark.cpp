...
int main() {
    ...
    if (single_cdf_ops_per_sec > 0 && single_pdf_ops_per_sec > 0) {
        if (single_pdf_ops_per_sec > single_cdf_ops_per_sec * 2) {
            ...
            std::cout << "PDF and CDF show similar performance, indicating efficient erf() "
                         "implementation.";
        }
    }
    std::cout << std::endl;
    ...
    std::cout << "├─ Box-Muller Sampling:         " << std::scientific
              << box_muller_sampling_ops_per_sec << " samples/sec" << std::endl;
    ...
    std::cout << "├─ Confidence Intervals (Mean):    " << std::scientific
              << confidence_interval_mean_ops_per_sec << " datapoints/sec" << std::endl;
    std::cout << "├─ Confidence Intervals (Variance):" << std::scientific
              << confidence_interval_variance_ops_per_sec << " datapoints/sec" << std::endl;
    std::cout << "├─ T-Tests (One Sample):          " << std::scientific << t_test_ops_per_sec
              << " datapoints/sec" << std::endl;
    std::cout << "├─ Method of Moments Estimation:   " << std::scientific
              << normality_test_ops_per_sec << " datapoints/sec" << std::endl;
    std::cout << "├─ Bayesian Parameter Estimation:  " << std::scientific
              << bayesian_estimation_ops_per_sec << " datapoints/sec" << std::endl;
    std::cout << "└─ Robust Parameter Estimation:    " << std::scientific
