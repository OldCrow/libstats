...
void demonstrate_creation_and_properties() {
    ...
    libstats::Poisson low_rate(2.0);    // Low rate
    libstats::Poisson moderate_rate(5.0); // Moderate rate  
    libstats::Poisson high_rate(10.0);   // Higher rate
    ...
    auto display_properties = [](const libstats::Poisson& dist, const std::string& name, double lambda) {
        std::cout << "📊 " << name << " properties:\n";
        std::cout << "     Rate parameter (λ): " << std::fixed << std::setprecision(4) << lambda << std::endl;
        ...
        std::cout << "     Variance: " << std::fixed << std::setprecision(4) << dist.getVariance() 
                  << " [Expected: " << lambda << "]" << std::endl;
        std::cout << "     Standard deviation: " << std::fixed << std::setprecision(4) << libstats::getStandardDeviation(dist) 
                  << " [Expected: " << std::sqrt(lambda) << "]" << std::endl;
...
void demonstrate_pmf_evaluations() {
    ...
    libstats::Poisson dist(3.0); // λ = 3 for demonstration
...
void demonstrate_cdf_operations() {
    ...
    libstats::Poisson dist(4.0); // λ = 4 for demonstration
...
void demonstrate_quantiles() {
    ...
    libstats::Poisson dist(6.0); // λ = 6 for demonstration
    ...
    for (double p : percentiles) {
        ...
        std::cout << "   " << std::fixed << std::setprecision(0) << (p * 100) << "th percentile: k=" << quantile
                  << " [P(X≤" << quantile << ")=" << std::fixed << std::setprecision(4) << actual_p << "]";
    ...
    std::cout << "   95% confidence interval: [" << lower_95 << ", " << upper_95 << "]" << std::endl;
    
    int lower_99 = static_cast<int>(dist.getQuantile(0.005));
    int upper_99 = static_cast<int>(dist.getQuantile(0.995));
    std::cout << "   99% confidence interval: [" << lower_99 << ", " << upper_99 << "]" << std::endl;
}
...
void demonstrate_sampling() {
    ...
    libstats::Poisson low_rate_dist(1.5);
    libstats::Poisson high_rate_dist(8.0);
...
void demonstrate_parameter_estimation() {
    ...
    libstats::Poisson true_dist(true_lambda);
    ...
    std::cout << "     Estimated λ (sample mean): " << std::fixed << std::setprecision(4) << estimated_lambda 
              << " [Should be close to " << true_lambda << "]" << std::endl;
    
    // Create distribution with estimated parameter
    libstats::Poisson estimated_dist(estimated_lambda);
    
    std::cout << "     Estimated mean: " << std::fixed << std::setprecision(4) << estimated_dist.getMean() 
              << " [Should be close to " << true_lambda << "]" << std::endl;
...
void demonstrate_batch_operations() {
    ...
    dist.getCumulativeProbability(std::span<const double>(input_values), std::span<double>(cdf_results));
