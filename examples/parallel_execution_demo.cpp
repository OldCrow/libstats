...
void demonstrate_platform_detection() {
    ...
    if (cpu_features.avx512f)
        std::cout << "AVX-512 (16x64-bit ops) ";
    if (cpu_features.avx2)
        std::cout << "AVX2 (4x64-bit ops) ";
    if (cpu_features.avx)
        std::cout << "AVX (4x64-bit ops) ";
    if (cpu_features.sse4_2)
        std::cout << "SSE4.2 (2x64-bit ops) ";
    if (cpu_features.neon)
        std::cout << "NEON (ARM SIMD) ";
    std::cout << std::endl;
...
void demonstrate_adaptive_grain_sizing() {
    ...
    std::cout << std::left << std::setw(12) << "Data Size" << std::setw(18) << "Memory-bound"
              << std::setw(18) << "Computation-bound" << std::setw(12) << "Mixed" << std::endl;
...
void demonstrate_thread_optimization() {
    ...
    std::cout << std::left << std::setw(15) << "Workload Size" << std::setw(18) << "Optimal Threads"
              << std::setw(12) << "Decision" << std::setw(20) << "Reasoning" << std::endl;
    ...
    for (auto workload : workload_sizes) {
        ...
        std::cout << std::setw(15) << workload << std::setw(18) << threads << std::setw(12)
                  << (should_parallel ? "Parallel" : "Serial") << std::setw(20) << reasoning
                  << std::endl;
    }

    std::cout << "\n   ℹ️ Small workloads use serial execution to avoid threading overhead"
              << std::endl;
...
void benchmark_parallel_operations() {
    ...
    // Reduce benchmark
    {
        ...
        std::cout << "  Reduce operation: " << duration.count() << " μs (result: " << std::fixed
                  << std::setprecision(2) << sum << ")" << std::endl;
    }
    ...
    // Count benchmark
    {
        ...
        std::cout << "  Count_if operation: " << duration.count() << " μs (result: " << count << ")"
                  << std::endl;
    }
}
    ...
    if (cpu_features.l3_cache_size > 0) {
        ...
        for (auto size : test_sizes) {
            ...
            std::cout << "  " << std::setw(10) << size << " elements (" << std::fixed
                      << std::setprecision(1) << cache_ratio << "x cache): grain = " << grain
                      << std::endl;
        }
    } else {
...
void demonstrate_parallel_batch_fit() {
    ...
    // --- Gaussian parallelBatchFit ------------------------------------------
    {
        ...
        std::vector<stats::GaussianDistribution> true_params(
            N_DATASETS, stats::GaussianDistribution::create(0.0, 1.0).unwrap());
        ...
        std::vector<stats::GaussianDistribution> seq_results(
            N_DATASETS, stats::GaussianDistribution::create(0.0, 1.0).unwrap());
        ...
        // Parallel fit
        std::vector<stats::GaussianDistribution> par_results(
            N_DATASETS, stats::GaussianDistribution::create(0.0, 1.0).unwrap());
        ...
        std::cout << "  Parallel fit:   " << t_par << " us"
                  ...
                  << static_cast<double>(t_seq) /
                         static_cast<double>(std::max<std::int64_t>(t_par, 1))
                  << "x)\n";
    ...
    // --- Exponential parallelBatchFit ---------------------------------------
    {
        ...
        std::vector<stats::ExponentialDistribution> par_results(
            N_DATASETS, stats::ExponentialDistribution::create(1.0).unwrap());
        auto t0 = std::chrono::high_resolution_clock::now();
